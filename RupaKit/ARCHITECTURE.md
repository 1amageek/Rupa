...
The T09 Agent-ready Mesh editing contract is defined by the system and package
design hierarchy in [`../DESIGN.md`](../DESIGN.md) and [`DESIGN.md`](DESIGN.md);
this file remains the package graph index and current-route summary.
...
| `EditorSession` applies the currently implemented `GeometrySourceCommand` through the injected Core applier and records its immutable result through the same `CADDocumentStore` generation and `CommandStack` history boundary as CAD commands. | Current Authored Mesh mutation does not bypass undo/redo, validation, evaluation invalidation, or transaction-revision ownership. |
...
Make Editable command validation occurs before staging: it re-evaluates the
current modeling source, compares the full immutable Mesh content, and checks
the source revision again after the asynchronous boundary so a concurrent
commit cannot publish stale promoted geometry.
Successful Authored Mesh edits mark superseded source blobs for collection;
Make Editable adds a new content-addressed source blob without replacing its CAD
source. The next package save reuses every unchanged content-addressed blob.
...
| Application project route | `ApplicationRoot`, `ApplicationProjectCoordinator`, `RupaKit.ProjectWorkspace`, and `ProjectViewSnapshot` | `ApplicationRoot` creates one `ProjectController`, one MainActor `ProjectWorkspace`, and one shared `ProjectWorkspaceOperationSequencer`. `MainView`, file/history commands, and production Agent registration use that same authority. Load, new, save, undo, redo, UI source/workspace actions, exact view recovery, file-path rebinding, dirty state, and cancellation are ordered through this boundary. `ProjectDocumentLifetimeID` resets transient presentation state when load or replacement changes the active document, including same-project-ID reloads. CAD-only, Mesh-only, and mixed packages redisplay through the same presentation projection without creating a second evaluator or editor authority. |
