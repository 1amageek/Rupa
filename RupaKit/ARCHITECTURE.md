...
| `ProjectController` publishes one `ProjectStateSnapshot` containing the matching document generation, transaction revision, publication sequence, presentation evaluation, optional CAD interaction context, selection, workspace state, package, and history flags. `ProjectWorkspace` converts it away from the main actor into a package-free `ProjectViewSnapshot` and publishes only increasing publication sequences on the main actor. | UI cannot combine geometry, CAD interaction, selection, or history from different publications; late asynchronous view construction cannot replace newer state; package bytes and source mutation authority do not enter observable UI state. |
