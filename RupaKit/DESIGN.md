...
geometry route, the production APIs consumed by the external T12 Agent CAD benchmark, and the CADAPI-D public modeling
...
- [RupaAgentCADBenchmark](Sources/RupaAgentCADBenchmark/DESIGN.md) (verification-only child)
...
below. The T12 runner, oracle, and tests stay in this package because their
response lifecycle consumes package-only encoding reservations. The JSON/CLI
and responsiveness tools live in the sibling
[RupaBenchmarks package](../RupaBenchmarks/DESIGN.md). The benchmark library is
exported for those tools; application targets never depend on it.
...
library. Other measurement tools belong to the sibling
[RupaBenchmarks package](../RupaBenchmarks/DESIGN.md).

The performance executable selects deferred kernel artifacts: body-count
validation uses exact B-Rep bodies, and incremental edits must rebuild one
feature while tessellating zero bodies. Its create/edit CLI smoke run verifies
these guards and the Core evaluation/history counters without claiming rendered
performance. CADAPI-100 target and oracle semantics are unaffected.
...
| [Benchmark JSON adapter](../RupaBenchmarks/Sources/RupaAgentCADBenchmarkJSONAdapter/DESIGN.md) | external consumer | versioned envelopes, context fingerprint, bounded decode, JSON candidate | Binds one external decision to the exact public context of one activated case. | It cannot import private expectations or accept a catalog-only case. |
| [Benchmark CLI](../RupaBenchmarks/Sources/RupaAgentCADBenchmarkCLI/DESIGN.md) | external consumer | dedicated request/evaluate process contract | Exposes the JSON adapter as `rupa-agent-cad-benchmark` without changing `rupa`. | It owns no envelope meaning, network transport, or project state. |
| [RupaResponsivenessBaseline design](../RupaBenchmarks/Sources/RupaResponsivenessBaseline/DESIGN.md) | external consumer | Versioned fixture, production-path measurement, and one verdict per acceptance row | Records the responsiveness baseline the RupaRendering acceptance table is judged against, using only public production contracts. | It owns no production behaviour and never relaxes a threshold or invents a value for an unobservable measure. |
| [Responsiveness baseline CLI design](../RupaBenchmarks/Sources/RupaResponsivenessBaselineCLI/DESIGN.md) | external consumer | dedicated measurement process contract | Exposes the baseline as `rupa-responsiveness-baseline` without changing `rupa`. | Its exit code is non-zero when measurement fails, when any acceptance row rejects, and when any row was not measured, with a distinct code per outcome. |
| [RupaResponsivenessFixtureDocument design](../RupaBenchmarks/Sources/RupaResponsivenessFixtureDocument/DESIGN.md) | external consumer | Fixture-to-project-package projection with reload verification | Materializes the measured fixture as a `.rupa` package so a signed application can be measured against the content the harness measured. | It is a separate target so `rupa-responsiveness-baseline` does not link the project and package stack, which would change the footprint its recorded baseline was taken against. |
| [Responsiveness fixture document CLI design](../RupaBenchmarks/Sources/RupaResponsivenessFixtureDocumentCLI/DESIGN.md) | external consumer | dedicated export process contract | Exposes the export as `rupa-responsiveness-fixture-document` without changing `rupa`. | It exits non-zero unless the writer reported a verified write, and prints the written document's identity so a later measurement can be attributed to it. |
...
| The external benchmark path is one-way: `RupaAgentCADBenchmark` -> JSON adapter -> dedicated CLI. It accepts only activated cases, fingerprints candidate-visible context, and never makes transport or candidate data authoritative. | [JSON adapter](../RupaBenchmarks/Sources/RupaAgentCADBenchmarkJSONAdapter/DESIGN.md), [benchmark CLI](../RupaBenchmarks/Sources/RupaAgentCADBenchmarkCLI/DESIGN.md) |
...
[JSON adapter](../RupaBenchmarks/Sources/RupaAgentCADBenchmarkJSONAdapter/DESIGN.md#runtime-flows),
and [benchmark CLI](../RupaBenchmarks/Sources/RupaAgentCADBenchmarkCLI/DESIGN.md#runtime-flows).
