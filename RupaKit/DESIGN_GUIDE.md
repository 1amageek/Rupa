...
| Scale | Canvas scale should stay visible through grid labels and a compact resolved-grid readout; configuration controls belong in document settings or inspector panels. Display-unit changes are formatting changes and must preserve physical ruler distances. Kilometer units are reserved for site-range readouts, while ordinary part and building edits should remain readable in μm, mm, cm, or m. |
