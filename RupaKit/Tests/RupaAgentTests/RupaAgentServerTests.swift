...
@Test func agentCapabilitiesExposeAutomationCommands() async throws {
    let capabilities = RupaAgentServer().capabilities()
...
@Test func agentMessageCodecRoundTripsEvaluateAndSaveResponses() async throws {
    ...
    let measureResponse = AgentResponse.measurement(
        RupaMeasurementResult(
            displayUnit: .millimeter,
            counts: RupaMeasurementResult.Counts(sourceFeatures: 2, sketches: 1, profiles: 1, solids: 1),
            totals: RupaMeasurementResult.Totals(
...
@Test func agentSavesOpenFileBackedSessionAndMarksClean() async throws {
    ...
    let server = RupaAgentServer()
    let sessionID = UUID()
    let session = EditorSession(document: try RupaDocumentFileService().load(from: url))
    _ = try session.execute(
...
@Test func mainActorAgentBridgeRoutesSessionMutations() async throws {
    let bridge = RupaMainActorAgentBridge()
    let sessionID = UUID()
    let session = EditorSession()
...
@Test(.timeLimit(.minutes(1))) func agentSocketListenerRoutesCommandThroughMainActorBridge() async throws {
    ...
    let socketPath = RupaAgentSocketPath(socketURL.path)
    let bridge = RupaMainActorAgentBridge()
    let sessionID = UUID()
    let session = EditorSession()
    bridge.register(session: session, id: sessionID)
...
@Test(.timeLimit(.minutes(1))) func agentSocketListenerStopRemovesSocketAndRejectsClient() async throws {
    ...
    let socketURL = temporaryDirectory.appendingPathComponent("rupa.sock")
    let listener = RupaAgentSocketListener(
        server: RupaAgentServer(),
        socketPath: RupaAgentSocketPath(socketURL.path)
    )
    let client = RupaAgentClient(socketPath: RupaAgentSocketPath(socketURL.path))
...
private func withRunningListener<T>(
    ...
    socketURL: URL,
    operation: (RupaAgentSocketListener, RupaAgentClient) async throws -> T
) async throws -> T {
    let socketPath = RupaAgentSocketPath(socketURL.path)
    let listener = RupaAgentSocketListener(
        server: server,
    ...
    let client = RupaAgentClient(socketPath: socketPath)
...
private func sendThroughDetachedClient(
    ...
    socketPath: RupaAgentSocketPath
