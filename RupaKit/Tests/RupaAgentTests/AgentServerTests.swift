...
@Test func agentCapabilityDescriptorsExposeDiscoveryAndMutationContracts() async throws {
    ...
    #expect(sweep.failureMode.contains("non-uniform affine, signed-axis, and convex quadrilateral bilinear point-guide rail deformation"))
    #expect(sweep.failureMode.contains("conflicting signed-axis rail guides"))
    #expect(sweep.failureMode.contains("flipped or self-intersecting bilinear quadrilateral rail guides"))
