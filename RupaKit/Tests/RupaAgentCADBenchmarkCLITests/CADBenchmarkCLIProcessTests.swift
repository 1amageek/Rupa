...
struct CADBenchmarkCLIProcessTests {
    ...
    @Test(.timeLimit(.minutes(2)))
    @MainActor
    func trn003UsesBoundedRequestFileAndStandardInputProductionRoutes() throws {
        let requestResult = try runCADBenchmarkCLI(["request", "TRN-003"])
        #expect(requestResult.terminationStatus == 0)
        #expect(requestResult.standardOutputData.count <= CADJSONAdapterSchema.maximumDocumentBytes)
        let request = try CADJSONBoundedCodec.decode(
            CADJSONRequestEnvelope.self,
        ...
        let exact = try responseData(for: "TRN-003", action: transform003Action())
        let fileResult = try withTemporaryData(exact) { path in
            try runCADBenchmarkCLI(["evaluate", "--response", path])
        }
        try assertRealizedEvaluation(fileResult, caseID: "TRN-003")

        let standardInputResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: exact
        )
        try assertRealizedEvaluation(standardInputResult, caseID: "TRN-003")

        let wrongOrder = try responseData(
            for: "TRN-003",
            action: transform003Action(translationY: -50, translationZ: 0)
        )
        let wrongResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: wrongOrder
        ...
        #expect(wrongResult.terminationStatus == 2)
        #expect(wrongEvaluation.caseID == "TRN-003")
        #expect(wrongEvaluation.result?.outcome == .invalidSubmission)
        #expect(wrongEvaluation.error == nil)
        #expect(isPrivateFree(wrongResult.standardOutput))
        ...
        let invalid = try responseData(
            for: "TRN-003",
            action: transform003Action(axis: CADDirection3D(x: 0, y: 0, z: 0))
        )
        let invalidResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: invalid
        ...
        #expect(invalidResult.terminationStatus == 2)
        #expect(invalidEvaluation.caseID == "TRN-003")
        #expect(invalidEvaluation.result?.outcome == .invalidSubmission)
        #expect(invalidEvaluation.error == nil)
        #expect(isPrivateFree(invalidResult.standardOutput))
    ...
    @Test(.timeLimit(.minutes(2)))
    @MainActor
    func trn004UsesBoundedRequestFileAndStandardInputProductionRoutes() throws {
        let requestResult = try runCADBenchmarkCLI(["request", "TRN-004"])
        #expect(requestResult.terminationStatus == 0)
        #expect(requestResult.standardOutputData.count <= CADJSONAdapterSchema.maximumDocumentBytes)
        let request = try CADJSONBoundedCodec.decode(
            CADJSONRequestEnvelope.self,
        ...
        let exact = try responseData(for: "TRN-004", action: transform004Action())
        let fileResult = try withTemporaryData(exact) { path in
            try runCADBenchmarkCLI(["evaluate", "--response", path])
        }
        try assertRealizedEvaluation(fileResult, caseID: "TRN-004")

        let standardInputResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: exact
        )
        try assertRealizedEvaluation(standardInputResult, caseID: "TRN-004")
        ...
        #expect(wrongResult.terminationStatus == 2)
        #expect(wrongEvaluation.caseID == "TRN-004")
        #expect(wrongEvaluation.result?.outcome == .invalidSubmission)
        #expect(wrongEvaluation.error == nil)
        #expect(isPrivateFree(wrongResult.standardOutput))
        ...
        let invalid = try responseData(
            for: "TRN-004",
            action: transform004Action(axis: CADDirection3D(x: 0, y: 0, z: 0))
        )
        let invalidResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: invalid
        ...
        #expect(invalidResult.terminationStatus == 2)
        #expect(invalidEvaluation.caseID == "TRN-004")
        #expect(invalidEvaluation.result?.outcome == .invalidSubmission)
        #expect(invalidEvaluation.error == nil)
        #expect(isPrivateFree(invalidResult.standardOutput))
    ...
    @Test(.timeLimit(.minutes(2)))
    @MainActor
    func trn005UsesBoundedRequestFileAndStandardInputProductionRoutes() throws {
        let requestResult = try runCADBenchmarkCLI(["request", "TRN-005"])
        #expect(requestResult.terminationStatus == 0)
        #expect(requestResult.standardOutputData.count <= CADJSONAdapterSchema.maximumDocumentBytes)
        let request = try CADJSONBoundedCodec.decode(
            CADJSONRequestEnvelope.self,
        ...
        let exact = try responseData(for: "TRN-005", action: transform005Action())
        let fileResult = try withTemporaryData(exact) { path in
            try runCADBenchmarkCLI(["evaluate", "--response", path])
        }
        try assertRealizedEvaluation(fileResult, caseID: "TRN-005")

        let standardInputResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: exact
        )
        try assertRealizedEvaluation(standardInputResult, caseID: "TRN-005")
        ...
        #expect(wrongResult.terminationStatus == 2)
        #expect(wrongEvaluation.caseID == "TRN-005")
        #expect(wrongEvaluation.result?.outcome == .invalidSubmission)
        #expect(wrongEvaluation.error == nil)
        #expect(isPrivateFree(wrongResult.standardOutput))
        ...
        let invalid = try responseData(
            for: "TRN-005",
            action: transform005Action(axis: CADDirection3D(x: 0, y: 0, z: 0))
        )
        let invalidResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: invalid
        ...
        #expect(invalidResult.terminationStatus == 2)
        #expect(invalidEvaluation.caseID == "TRN-005")
        #expect(invalidEvaluation.result?.outcome == .invalidSubmission)
        #expect(invalidEvaluation.error == nil)
        #expect(isPrivateFree(invalidResult.standardOutput))
    ...
    @Test(.timeLimit(.minutes(2)))
    @MainActor
    func trn006UsesBoundedRequestFileAndStandardInputProductionRoutes() throws {
        let requestResult = try runCADBenchmarkCLI(["request", "TRN-006"])
        #expect(requestResult.terminationStatus == 0)
        #expect(requestResult.standardOutputData.count <= CADJSONAdapterSchema.maximumDocumentBytes)
        let request = try CADJSONBoundedCodec.decode(
            CADJSONRequestEnvelope.self,
        ...
        let exact = try responseData(for: "TRN-006", action: transform006Action())
        let fileResult = try withTemporaryData(exact) { path in
            try runCADBenchmarkCLI(["evaluate", "--response", path])
        }
        try assertRealizedEvaluation(fileResult, caseID: "TRN-006")

        let standardInputResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: exact
        )
        try assertRealizedEvaluation(standardInputResult, caseID: "TRN-006")

        let inverseAxis = try responseData(
            for: "TRN-006",
            action: transform006Action(axis: CADDirection3D(x: -1, y: 0, z: 0))
        )
        let inverseResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: inverseAxis
        )
        let inverseEvaluation = try CADJSONBoundedCodec.decode(
            CADJSONEvaluationEnvelope.self,
            from: inverseResult.standardOutputData
        )
        #expect(inverseResult.terminationStatus == 2)
        #expect(inverseEvaluation.caseID == "TRN-006")
        #expect(inverseEvaluation.result?.outcome == .invalidSubmission)
        #expect(inverseEvaluation.error == nil)
        #expect(isPrivateFree(inverseResult.standardOutput))

        let invalid = try responseData(
            for: "TRN-006",
            action: transform006Action(axis: CADDirection3D(x: 0, y: 0, z: 0))
        )
        let invalidResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: invalid
        ...
        #expect(invalidResult.terminationStatus == 2)
        #expect(invalidEvaluation.caseID == "TRN-006")
        #expect(invalidEvaluation.result?.outcome == .invalidSubmission)
        #expect(invalidEvaluation.error == nil)
        #expect(isPrivateFree(invalidResult.standardOutput))
    ...
    @Test(.timeLimit(.minutes(2)))
    @MainActor
    func trn007UsesBoundedRequestFileAndStandardInputProductionRoutes() throws {
        let requestResult = try runCADBenchmarkCLI(["request", "TRN-007"])
        #expect(requestResult.terminationStatus == 0)
        #expect(requestResult.standardOutputData.count <= CADJSONAdapterSchema.maximumDocumentBytes)
        let request = try CADJSONBoundedCodec.decode(
            CADJSONRequestEnvelope.self,
        ...
        let exact = try responseData(for: "TRN-007", action: transform007Action())
        let fileResult = try withTemporaryData(exact) { path in
            try runCADBenchmarkCLI(["evaluate", "--response", path])
        }
        try assertRealizedEvaluation(fileResult, caseID: "TRN-007")

        let standardInputResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: exact
        )
        try assertRealizedEvaluation(standardInputResult, caseID: "TRN-007")

        let wrongAngle = try responseData(
            for: "TRN-007",
            action: transform007Action(rotationDegrees: 90)
        )
        let wrongAngleResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: wrongAngle
        )
        let wrongAngleEvaluation = try CADJSONBoundedCodec.decode(
            CADJSONEvaluationEnvelope.self,
            from: wrongAngleResult.standardOutputData
        )
        #expect(wrongAngleResult.terminationStatus == 2)
        ...
        #expect(isPrivateFree(wrongAngleResult.standardOutput))

        let invalid = try responseData(
            for: "TRN-007",
            action: transform007Action(axis: CADDirection3D(x: 0, y: 0, z: 0))
        )
        let invalidResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: invalid
        ...
        #expect(invalidResult.terminationStatus == 2)
        #expect(invalidEvaluation.caseID == "TRN-007")
        #expect(invalidEvaluation.result?.outcome == .invalidSubmission)
        #expect(invalidEvaluation.error == nil)
        #expect(isPrivateFree(invalidResult.standardOutput))
    ...
    @Test(.timeLimit(.minutes(2)))
    @MainActor
    func trn008UsesBoundedRequestFileAndStandardInputProductionRoutes() throws {
        let requestResult = try runCADBenchmarkCLI(["request", "TRN-008"])
        #expect(requestResult.terminationStatus == 0)
        #expect(requestResult.standardOutputData.count <= CADJSONAdapterSchema.maximumDocumentBytes)
        let request = try CADJSONBoundedCodec.decode(
            CADJSONRequestEnvelope.self,
        ...
        let exact = try responseData(for: "TRN-008", action: transform008Action())
        let fileResult = try withTemporaryData(exact) { path in
            try runCADBenchmarkCLI(["evaluate", "--response", path])
        }
        try assertRealizedEvaluation(fileResult, caseID: "TRN-008")

        let standardInputResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: exact
        )
        try assertRealizedEvaluation(standardInputResult, caseID: "TRN-008")
        ...
        let wrongOrderResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: wrongOrder
        )
        let wrongOrderEvaluation = try CADJSONBoundedCodec.decode(
            CADJSONEvaluationEnvelope.self,
            from: wrongOrderResult.standardOutputData
        )
        #expect(wrongOrderResult.terminationStatus == 2)
        ...
        #expect(isPrivateFree(wrongOrderResult.standardOutput))

        let invalid = try responseData(
            for: "TRN-008",
            action: transform008Action(axis: CADDirection3D(x: 0, y: 0, z: 0))
        )
        let invalidResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: invalid
        ...
        #expect(invalidResult.terminationStatus == 2)
        #expect(invalidEvaluation.caseID == "TRN-008")
        #expect(invalidEvaluation.result?.outcome == .invalidSubmission)
        #expect(invalidEvaluation.error == nil)
        #expect(isPrivateFree(invalidResult.standardOutput))
    ...
    @Test(.timeLimit(.minutes(2)))
    @MainActor
    func cmp001UsesBoundedRequestFileAndStandardInputProductionRoutes() throws {
        let requestResult = try runCADBenchmarkCLI(["request", "CMP-001"])
        #expect(requestResult.terminationStatus == 0)
        #expect(requestResult.standardOutputData.count <= CADJSONAdapterSchema.maximumDocumentBytes)
        let request = try CADJSONBoundedCodec.decode(
            CADJSONRequestEnvelope.self,
        ...
        let exact = try responseData(for: "CMP-001", action: compound001Action())
        ...
        try assertRealizedEvaluation(fileResult, caseID: "CMP-001")

        let standardInputResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: exact
        )
        try assertRealizedEvaluation(standardInputResult, caseID: "CMP-001")

        let wrong = try responseData(
            for: "CMP-001",
            action: compound001Action(baseOriginZ: 1)
        )
        let wrongResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: wrong
        ...
        let invalid = try responseData(
            for: "CMP-001",
            action: compound001Action(postRadius: 0)
        )
        let invalidResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: invalid
    ...
    @Test(.timeLimit(.minutes(2)))
    @MainActor
    func cmp002UsesBoundedRequestFileAndStandardInputProductionRoutes() throws {
        let requestResult = try runCADBenchmarkCLI(["request", "CMP-002"])
        #expect(requestResult.terminationStatus == 0)
        #expect(requestResult.standardOutputData.count <= CADJSONAdapterSchema.maximumDocumentBytes)
        let request = try CADJSONBoundedCodec.decode(
            CADJSONRequestEnvelope.self,
        ...
        let exact = try responseData(for: "CMP-002", action: compound002Action())
        #expect(exact.count <= CADJSONAdapterSchema.maximumDocumentBytes)
        let exactText = String(decoding: exact, as: UTF8.self)
        #expect(exactText.contains("\"kind\":\"compound\""))
        #expect(isPrivateFree(exactText))
        ...
        try assertRealizedEvaluation(fileResult, caseID: "CMP-002")

        let standardInputResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: exact
        )
        try assertRealizedEvaluation(standardInputResult, caseID: "CMP-002")

        let wrong = try responseData(
            for: "CMP-002",
            action: compound002Action(leftOriginX: 15, rightOriginX: -40)
        )
        let wrongResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: wrong
        ...
        let invalid = try responseData(
            for: "CMP-002",
            action: compound002Action(rightWidth: 0)
        )
        let invalidResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: invalid
    ...
    @Test(.timeLimit(.minutes(2)))
    @MainActor
    func cmp003UsesBoundedRequestFileAndStandardInputProductionRoutes() throws {
        let requestResult = try runCADBenchmarkCLI(["request", "CMP-003"])
        #expect(requestResult.terminationStatus == 0)
        #expect(requestResult.standardOutputData.count <= CADJSONAdapterSchema.maximumDocumentBytes)
        let request = try CADJSONBoundedCodec.decode(
            CADJSONRequestEnvelope.self,
        ...
        let exact = try responseData(for: "CMP-003", action: compound003Action())
        #expect(exact.count <= CADJSONAdapterSchema.maximumDocumentBytes)
        let exactText = String(decoding: exact, as: UTF8.self)
        #expect(exactText.contains("\"kind\":\"compound\""))
        #expect(isPrivateFree(exactText))
        ...
        try assertRealizedEvaluation(fileResult, caseID: "CMP-003")

        let standardInputResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: exact
        )
        try assertRealizedEvaluation(standardInputResult, caseID: "CMP-003")

        let wrong = try responseData(
            for: "CMP-003",
            action: compound003Action(swappingPayloads: true)
        )
        let wrongResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: wrong
        ...
        let invalid = try responseData(
            for: "CMP-003",
            action: compound003Action(collarRadius: 0)
        )
        let invalidResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: invalid
    ...
    @Test(.timeLimit(.minutes(2)))
    @MainActor
    func cmp004UsesBoundedRequestFileAndStandardInputProductionRoutes() throws {
        let requestResult = try runCADBenchmarkCLI(["request", "CMP-004"])
        #expect(requestResult.terminationStatus == 0)
        #expect(requestResult.standardOutputData.count <= CADJSONAdapterSchema.maximumDocumentBytes)
        let request = try CADJSONBoundedCodec.decode(
            CADJSONRequestEnvelope.self,
        ...
        let exact = try responseData(for: "CMP-004", action: compound004Action())
        #expect(exact.count <= CADJSONAdapterSchema.maximumDocumentBytes)
        let exactText = String(decoding: exact, as: UTF8.self)
        #expect(exactText.contains("\"kind\":\"compound\""))
        #expect(isPrivateFree(exactText))
        ...
        try assertRealizedEvaluation(fileResult, caseID: "CMP-004")

        let standardInputResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: exact
        )
        try assertRealizedEvaluation(standardInputResult, caseID: "CMP-004")

        let wrong = try responseData(
            for: "CMP-004",
            action: compound004Action(swappingPinPayloads: true)
        )
        let wrongResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: wrong
        ...
        let invalid = try responseData(
            for: "CMP-004",
            action: compound004Action(pinBRadius: 0)
        )
        let invalidResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: invalid
    ...
    @Test(.timeLimit(.minutes(2)))
    @MainActor
    func cmp006UsesBoundedRequestFileAndStandardInputProductionRoutes() throws {
        let requestResult = try runCADBenchmarkCLI(["request", "CMP-006"])
        #expect(requestResult.terminationStatus == 0)
        #expect(requestResult.standardOutputData.count <= CADJSONAdapterSchema.maximumDocumentBytes)
        let request = try CADJSONBoundedCodec.decode(
            CADJSONRequestEnvelope.self,
        ...
        let exact = try responseData(for: "CMP-006", action: compound006Action())
        #expect(exact.count <= CADJSONAdapterSchema.maximumDocumentBytes)
        let exactText = String(decoding: exact, as: UTF8.self)
        #expect(exactText.contains("\"kind\":\"compound\""))
        #expect(isPrivateFree(exactText))
        ...
        try assertRealizedEvaluation(fileResult, caseID: "CMP-006")

        let standardInputResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: exact
        )
        try assertRealizedEvaluation(standardInputResult, caseID: "CMP-006")

        let wrong = try responseData(
            for: "CMP-006",
            action: compound006Action(swappingArmPayloads: true)
        )
        let wrongResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: wrong
        ...
        let invalid = try responseData(
            for: "CMP-006",
            action: compound006Action(armBWidth: 0)
        )
        let invalidResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: invalid
    ...
    @Test(.timeLimit(.minutes(2)))
    @MainActor
    func cmp007UsesBoundedRequestFileAndStandardInputProductionRoutes() throws {
        let requestResult = try runCADBenchmarkCLI(["request", "CMP-007"])
        #expect(requestResult.terminationStatus == 0)
        #expect(requestResult.standardOutputData.count <= CADJSONAdapterSchema.maximumDocumentBytes)
        let request = try CADJSONBoundedCodec.decode(
            CADJSONRequestEnvelope.self,
        ...
        let exact = try responseData(for: "CMP-007", action: compound007Action())
        #expect(exact.count <= CADJSONAdapterSchema.maximumDocumentBytes)
        let exactText = String(decoding: exact, as: UTF8.self)
        #expect(exactText.contains("\"kind\":\"compound\""))
        #expect(isPrivateFree(exactText))
        ...
        try assertRealizedEvaluation(fileResult, caseID: "CMP-007")

        let standardInputResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: exact
        )
        try assertRealizedEvaluation(standardInputResult, caseID: "CMP-007")

        let wrong = try responseData(
            for: "CMP-007",
            action: compound007Action(boreBaseX: 26)
        )
        let wrongResult = try runCADBenchmarkCLI(
            ["evaluate", "--response", "-"],
            standardInput: wrong
        ...
        for invalidAction in [
            ...
            let invalid = try responseData(for: "CMP-007", action: invalidAction)
            let invalidResult = try runCADBenchmarkCLI(
                ["evaluate", "--response", "-"],
                standardInput: invalid
            )
    ...
    @Test(.timeLimit(.minutes(2)))
    @MainActor
    func requestEmitsEveryBoundedCatalogObjectInLexicalOrder() throws {
        let catalog = try CADBenchmarkCatalog()
        #expect(catalog.caseIDs.count == 100)
        for rawCaseID in catalog.caseIDs.map(\.rawValue) {
    ...
    @Test(.timeLimit(.minutes(2)))
    @MainActor
    func malformedOversizeSchemaFingerprintAndNonActionResponsesHaveStableOutcomes() throws {
