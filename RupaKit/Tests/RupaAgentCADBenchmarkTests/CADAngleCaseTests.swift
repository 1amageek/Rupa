...
struct CADAngleCaseTests {
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func ang013CreatesTheNinetyDegreePairOnCanonicalXZPlane() async throws {
        let result = try await CADAngleCaseRunner(case: .ang013).runReference()

        try result.validate()
        #expect(result.caseID == "ANG-013")
        #expect(result.outcome == .realized)
        #expect(result.candidateResults.map(\.stepIndex) == [0, 1])
        #expect(result.candidateResults.allSatisfy { $0.status == .published })
        #expect(result.roleBindings?.bindings.map(\.role) == ["first-line", "second-line"])
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func ang013RejectsReversedSecondDirectionAfterOnePublicationWithoutRetry() async throws {
        let result = try await CADAngleCaseRunner(case: .ang013).run(
            action: angle013Action(
                secondEnd: CADPoint3D(x: -15, y: 25, z: 30, unit: .millimeter)
            )
        )
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func ang013RejectsOffPlaneEndpointBeforePublication() async throws {
        let result = try await CADAngleCaseRunner(case: .ang013).run(
            action: angle013Action(
                firstEnd: CADPoint3D(x: 35, y: 27, z: 180, unit: .millimeter)
            )
        )
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func ang013TimeoutRetainsAtomicCleanupEvidence() async throws {
        let result = try await CADAngleCaseRunner(
            case: .ang013,
            timeoutWallNanoseconds: 1
        ).runReference()
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func ang014CreatesTheOneHundredTwentyDegreePairOnCanonicalYZPlane() async throws {
        let result = try await CADAngleCaseRunner(case: .ang014).runReference()

        try result.validate()
        #expect(result.caseID == "ANG-014")
        #expect(result.outcome == .realized)
        #expect(result.candidateResults.map(\.stepIndex) == [0, 1])
        #expect(result.candidateResults.allSatisfy { $0.status == .published })
        #expect(result.roleBindings?.bindings.map(\.role) == ["first-line", "second-line"])
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func ang014RejectsTranslatedSecondEndpointAfterOnePublicationWithoutRetry() async throws {
        let result = try await CADAngleCaseRunner(case: .ang014).run(
            action: angle014Action(
                secondEnd: CADPoint3D(
                    x: -25,
                    y: -82.5 + 1,
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func ang014RejectsOffPlaneEndpointBeforePublication() async throws {
        let result = try await CADAngleCaseRunner(case: .ang014).run(
            action: angle014Action(
                firstEnd: CADPoint3D(x: -23, y: 105, z: 275, unit: .millimeter)
            )
        )
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func ang014TimeoutRetainsAtomicCleanupEvidence() async throws {
        let result = try await CADAngleCaseRunner(
            case: .ang014,
            timeoutWallNanoseconds: 1
        ).runReference()
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func ang015CreatesTheOneHundredThirtyFiveDegreePairOnCanonicalXZPlane() async throws {
        let result = try await CADAngleCaseRunner(case: .ang015).runReference()

        try result.validate()
        #expect(result.caseID == "ANG-015")
        #expect(result.outcome == .realized)
        #expect(result.candidateResults.map(\.stepIndex) == [0, 1])
        #expect(result.candidateResults.allSatisfy { $0.status == .published })
        #expect(result.roleBindings?.bindings.map(\.role) == ["first-line", "second-line"])
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func ang015RejectsReversedSecondDirectionAfterOnePublicationWithoutRetry() async throws {
        let result = try await CADAngleCaseRunner(case: .ang015).run(
            action: angle015Action(
                secondEnd: CADPoint3D(
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func ang015RejectsOffPlaneEndpointBeforePublication() async throws {
        let result = try await CADAngleCaseRunner(case: .ang015).run(
            action: angle015Action(
                firstEnd: CADPoint3D(x: 140, y: -38, z: 325, unit: .millimeter)
            )
        )
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func ang015TimeoutRetainsAtomicCleanupEvidence() async throws {
        let result = try await CADAngleCaseRunner(
            case: .ang015,
            timeoutWallNanoseconds: 1
        ).runReference()
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func ang016CreatesTheOneHundredFiftyDegreePairOnCanonicalYZPlane() async throws {
        let result = try await CADAngleCaseRunner(case: .ang016).runReference()

        try result.validate()
        #expect(result.caseID == "ANG-016")
        #expect(result.outcome == .realized)
        #expect(result.candidateResults.map(\.stepIndex) == [0, 1])
        #expect(result.candidateResults.allSatisfy { $0.status == .published })
        #expect(result.roleBindings?.bindings.map(\.role) == ["first-line", "second-line"])
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func ang016RejectsWrongOneHundredTwentyDegreeSecondDirectionAfterOnePublicationWithoutRetry() async throws {
        let result = try await CADAngleCaseRunner(case: .ang016).run(
            action: angle016Action(
                secondEnd: CADPoint3D(
                    x: 60,
                    y: 60 - 375 * 0.5,
                    z: 425 + 375 * 0.866025403784,
                    unit: .millimeter
                )
            )
        )
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func ang016RejectsOffPlaneEndpointBeforePublication() async throws {
        let result = try await CADAngleCaseRunner(case: .ang016).run(
            action: angle016Action(
                firstEnd: CADPoint3D(x: 62, y: 185, z: 425, unit: .millimeter)
            )
        )
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func ang016TimeoutRetainsAtomicCleanupEvidence() async throws {
        let result = try await CADAngleCaseRunner(
            case: .ang016,
            timeoutWallNanoseconds: 1
        ).runReference()
    ...
    private func angle013Action(
        firstStart: CADPoint3D = CADPoint3D(x: -15, y: 25, z: 180, unit: .millimeter),
        firstEnd: CADPoint3D = CADPoint3D(x: 35, y: 25, z: 180, unit: .millimeter),
        secondStart: CADPoint3D = CADPoint3D(x: -15, y: 25, z: 180, unit: .millimeter),
        secondEnd: CADPoint3D = CADPoint3D(x: -15, y: 25, z: 330, unit: .millimeter)
    ) -> CADCandidateAction {
        .automation(
            .sketch(
                .angle(
    ...
    private func angle014Action(
        firstStart: CADPoint3D = CADPoint3D(x: -25, y: 30, z: 275, unit: .millimeter),
        firstEnd: CADPoint3D = CADPoint3D(x: -25, y: 105, z: 275, unit: .millimeter),
        secondStart: CADPoint3D = CADPoint3D(x: -25, y: 30, z: 275, unit: .millimeter),
        secondEnd: CADPoint3D = CADPoint3D(
            x: -25,
            y: 30 - 225 * 0.5,
            z: 275 + 225 * 0.866025403784,
    ...
    private func angle015Action(
        firstStart: CADPoint3D = CADPoint3D(x: 40, y: -40, z: 325, unit: .millimeter),
        firstEnd: CADPoint3D = CADPoint3D(x: 140, y: -40, z: 325, unit: .millimeter),
        secondStart: CADPoint3D = CADPoint3D(x: 40, y: -40, z: 325, unit: .millimeter),
        secondEnd: CADPoint3D = CADPoint3D(
            x: 40 - 300 * 0.707106781187,
            y: -40,
            z: 325 + 300 * 0.707106781187,
    ...
    private func angle016Action(
        firstStart: CADPoint3D = CADPoint3D(x: 60, y: 60, z: 425, unit: .millimeter),
        firstEnd: CADPoint3D = CADPoint3D(x: 60, y: 185, z: 425, unit: .millimeter),
        secondStart: CADPoint3D = CADPoint3D(x: 60, y: 60, z: 425, unit: .millimeter),
        secondEnd: CADPoint3D = CADPoint3D(
            x: 60,
