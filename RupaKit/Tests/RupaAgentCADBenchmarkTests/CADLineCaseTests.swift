...
func activatedLineBoundaryContainsExactlyReviewedCases() throws {
    #expect(CADActivatedLineCase.allCases.map(\.rawValue) == ["LIN-001", "LIN-002", "LIN-003", "LIN-004"])

    do {
        _ = try CADActivatedLineCase(caseID: "LIN-005")
        Issue.record("LIN-005 must remain inactive until its own vertical gate.")
    } catch let error as CADBenchmarkError {
        guard case .invalidCaseID("LIN-005") = error else {
            Issue.record("Unexpected typed error: \(error)")
            return
        }
    }
