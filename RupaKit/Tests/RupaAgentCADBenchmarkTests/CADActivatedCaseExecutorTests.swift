...
struct CADActivatedCaseExecutorTests {
    ...
    func executorActivatesOnlyReviewedCasesThroughSph002() throws {
    ...
    func activatedLineAndRectangleCandidatesUseProductionLifecycle() async throws {
        ...
        let translatedBox = try await executor.evaluate(
            caseID: "BOX-002",
            candidate: CADBoxReferenceCandidate()
        )
        #expect(translatedBox.id == "BOX-002")
        #expect(translatedBox.category == .box)
        #expect(translatedBox.outcome == .realized)
        try translatedBox.validate()
        let translatedBoxEncoded = try canonicalJSON(translatedBox)
        for forbidden in ["FeatureID", "diagnostics", "telemetry", "expectation", "workspace"] {
            #expect(translatedBoxEncoded.contains(forbidden) == false)
        }

        let rectangularBox = try await executor.evaluate(
            caseID: "BOX-003",
            candidate: CADBoxReferenceCandidate()
        )
        #expect(rectangularBox.id == "BOX-003")
        #expect(rectangularBox.category == .box)
        #expect(rectangularBox.outcome == .realized)
        try rectangularBox.validate()
        let rectangularBoxEncoded = try canonicalJSON(rectangularBox)
        for forbidden in ["FeatureID", "diagnostics", "telemetry", "expectation", "workspace"] {
            #expect(rectangularBoxEncoded.contains(forbidden) == false)
        }

        let translatedRectangularBox = try await executor.evaluate(
            caseID: "BOX-004",
            candidate: CADBoxReferenceCandidate()
        )
        #expect(translatedRectangularBox.id == "BOX-004")
        #expect(translatedRectangularBox.category == .box)
        #expect(translatedRectangularBox.outcome == .realized)
        try translatedRectangularBox.validate()
        let translatedRectangularBoxEncoded = try canonicalJSON(translatedRectangularBox)
        for forbidden in ["FeatureID", "diagnostics", "telemetry", "expectation", "workspace"] {
            #expect(translatedRectangularBoxEncoded.contains(forbidden) == false)
        }

        let largeRectangularBox = try await executor.evaluate(
            caseID: "BOX-005",
            candidate: CADBoxReferenceCandidate()
        )
        #expect(largeRectangularBox.id == "BOX-005")
        #expect(largeRectangularBox.category == .box)
        #expect(largeRectangularBox.outcome == .realized)
        try largeRectangularBox.validate()
        let largeRectangularBoxEncoded = try canonicalJSON(largeRectangularBox)
        for forbidden in ["FeatureID", "diagnostics", "telemetry", "expectation", "workspace"] {
            #expect(largeRectangularBoxEncoded.contains(forbidden) == false)
        }

        let meterScaleBox = try await executor.evaluate(
            caseID: "BOX-006",
            candidate: CADBoxReferenceCandidate()
        )
        #expect(meterScaleBox.id == "BOX-006")
        #expect(meterScaleBox.category == .box)
        #expect(meterScaleBox.outcome == .realized)
        try meterScaleBox.validate()
        let meterScaleBoxEncoded = try canonicalJSON(meterScaleBox)
        for forbidden in ["FeatureID", "diagnostics", "telemetry", "expectation", "workspace"] {
            #expect(meterScaleBoxEncoded.contains(forbidden) == false)
        }

        let imperialBox = try await executor.evaluate(
            caseID: "BOX-007",
            candidate: CADBoxReferenceCandidate()
        )
        #expect(imperialBox.id == "BOX-007")
        #expect(imperialBox.category == .box)
        #expect(imperialBox.outcome == .realized)
        try imperialBox.validate()
        let imperialBoxEncoded = try canonicalJSON(imperialBox)
        for forbidden in ["FeatureID", "diagnostics", "telemetry", "expectation", "workspace"] {
            #expect(imperialBoxEncoded.contains(forbidden) == false)
        }

        let translatedCube = try await executor.evaluate(
            caseID: "BOX-008",
            candidate: CADBoxReferenceCandidate()
        )
        #expect(translatedCube.id == "BOX-008")
        #expect(translatedCube.category == .box)
        #expect(translatedCube.outcome == .realized)
        try translatedCube.validate()
        let translatedCubeEncoded = try canonicalJSON(translatedCube)
        for forbidden in ["FeatureID", "diagnostics", "telemetry", "expectation", "workspace"] {
            #expect(translatedCubeEncoded.contains(forbidden) == false)
        }

        let negativePlacementCube = try await executor.evaluate(
            caseID: "BOX-009",
            candidate: CADBoxReferenceCandidate()
        )
        #expect(negativePlacementCube.id == "BOX-009")
        #expect(negativePlacementCube.category == .box)
        #expect(negativePlacementCube.outcome == .realized)
        try negativePlacementCube.validate()
        let negativePlacementCubeEncoded = try canonicalJSON(negativePlacementCube)
        for forbidden in ["FeatureID", "diagnostics", "telemetry", "expectation", "workspace"] {
            #expect(negativePlacementCubeEncoded.contains(forbidden) == false)
        }

        let rectangularSolid = try await executor.evaluate(
            caseID: "BOX-010",
            candidate: CADBoxReferenceCandidate()
        )
        #expect(rectangularSolid.id == "BOX-010")
        #expect(rectangularSolid.category == .box)
        #expect(rectangularSolid.outcome == .realized)
        try rectangularSolid.validate()
        let rectangularSolidEncoded = try canonicalJSON(rectangularSolid)
        for forbidden in ["FeatureID", "diagnostics", "telemetry", "expectation", "workspace"] {
            #expect(rectangularSolidEncoded.contains(forbidden) == false)
        }

        let metreCube = try await executor.evaluate(
            caseID: "BOX-011",
            candidate: CADBoxReferenceCandidate()
        )
        #expect(metreCube.id == "BOX-011")
        #expect(metreCube.category == .box)
        #expect(metreCube.outcome == .realized)
        try metreCube.validate()
        let metreCubeEncoded = try canonicalJSON(metreCube)
        for forbidden in ["FeatureID", "diagnostics", "telemetry", "expectation", "workspace"] {
            #expect(metreCubeEncoded.contains(forbidden) == false)
        }

        let millimeterSolid = try await executor.evaluate(
            caseID: "BOX-012",
            candidate: CADBoxReferenceCandidate()
        )
        #expect(millimeterSolid.id == "BOX-012")
        #expect(millimeterSolid.category == .box)
        #expect(millimeterSolid.outcome == .realized)
        try millimeterSolid.validate()
        let millimeterSolidEncoded = try canonicalJSON(millimeterSolid)
        for forbidden in ["FeatureID", "diagnostics", "telemetry", "expectation", "workspace"] {
            #expect(millimeterSolidEncoded.contains(forbidden) == false)
        }
    }
    ...
    func compound001UsesThePublicExecutorRouteAndAllMemberCapabilities() async throws {
        let executor = DefaultCADActivatedCaseExecutor()
        let context = try executor.context(for: "CMP-001")
        #expect(context.challenge.category == .compound)
        #expect(context.challenge.requiredCapability.id == "cad.assembly.compound")
        #expect(context.capabilities.statuses.count == 1)
        #expect(
    ...
    @MainActor
    @Test(.timeLimit(.minutes(2)))
    func compoundCandidateFailureIsTypedForEveryActivatedCase() async throws {
        let executor = DefaultCADActivatedCaseExecutor()
        ...
        for activatedCase in CADActivatedCompoundCase.allCases {
            do {
                ...
                Issue.record("Compound candidate failure must be thrown.")
            } catch let error as CADActivatedCaseExecutorError {
                #expect(error == .candidateFailure(activatedCase.caseID))
            }
        }
    }
    ...
    @MainActor
    @Test(.timeLimit(.minutes(2)))
    func compound002UsesThePublicExecutorRoute() async throws {
        let executor = DefaultCADActivatedCaseExecutor()
        let context = try executor.context(for: "CMP-002")
        #expect(context.challenge.category == .compound)
        #expect(context.challenge.requiredCapability.id == "cad.assembly.compound")
        #expect(
            context.capabilities.status(for: context.challenge.requiredCapability)?.available
    ...
    @MainActor
    @Test(.timeLimit(.minutes(2)))
    func compound003UsesThePublicExecutorRoute() async throws {
        let executor = DefaultCADActivatedCaseExecutor()
        let context = try executor.context(for: "CMP-003")
        #expect(context.challenge.category == .compound)
        #expect(context.challenge.requiredCapability.id == "cad.assembly.compound")
        #expect(
            context.capabilities.status(for: context.challenge.requiredCapability)?.available
    ...
    @MainActor
    @Test(.timeLimit(.minutes(2)))
    func compound004UsesThePublicExecutorRoute() async throws {
        let executor = DefaultCADActivatedCaseExecutor()
        let context = try executor.context(for: "CMP-004")
        #expect(context.challenge.category == .compound)
        #expect(context.challenge.requiredCapability.id == "cad.assembly.compound")
        #expect(
            context.capabilities.status(for: context.challenge.requiredCapability)?.available
    ...
    @MainActor
    @Test(.timeLimit(.minutes(2)))
    func compound005UsesThePublicExecutorRoute() async throws {
        let executor = DefaultCADActivatedCaseExecutor()
        let context = try executor.context(for: "CMP-005")
        #expect(context.challenge.category == .compound)
        #expect(context.challenge.requiredCapability.id == "cad.assembly.compound")
        #expect(
            context.capabilities.status(for: context.challenge.requiredCapability)?.available
    ...
    @MainActor
    @Test(.timeLimit(.minutes(2)))
    func compound006UsesThePublicExecutorRoute() async throws {
        let executor = DefaultCADActivatedCaseExecutor()
        let context = try executor.context(for: "CMP-006")
        #expect(context.challenge.category == .compound)
        #expect(context.challenge.requiredCapability.id == "cad.assembly.compound")
        #expect(
            context.capabilities.status(for: context.challenge.requiredCapability)?.available
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func transform002UsesTheProductionLifecycle() async throws {
        let executor = DefaultCADActivatedCaseExecutor()
        let context = try executor.context(for: "TRN-002")
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func transform003UsesTheProductionLifecycle() async throws {
        let executor = DefaultCADActivatedCaseExecutor()
        let context = try executor.context(for: "TRN-003")
        #expect(context.challenge.category == .transform)
        #expect(context.challenge.instruction.contains("circle"))
        let result = try await executor.evaluate(
            caseID: "TRN-003",
            candidate: CADTransformReferenceCandidate()
        )

        #expect(result.id == "TRN-003")
        #expect(result.category == .transform)
        #expect(result.outcome == .realized)
        try result.validate()
    }
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func transform004UsesTheProductionLifecycle() async throws {
        let executor = DefaultCADActivatedCaseExecutor()
        let context = try executor.context(for: "TRN-004")
        #expect(context.challenge.category == .transform)
        #expect(context.challenge.instruction.contains("box"))
        let result = try await executor.evaluate(
            caseID: "TRN-004",
            candidate: CADTransformReferenceCandidate()
        )

        #expect(result.id == "TRN-004")
        #expect(result.category == .transform)
        #expect(result.outcome == .realized)
        try result.validate()
    }
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func transform005UsesTheProductionLifecycleAndExactSolidCounts() async throws {
        let executor = DefaultCADActivatedCaseExecutor()
        let context = try executor.context(for: "TRN-005")
        #expect(context.challenge.category == .transform)
        #expect(context.challenge.instruction.contains("cylinder"))
        let result = try await executor.evaluate(
            caseID: "TRN-005",
            candidate: CADTransformReferenceCandidate()
        )

        #expect(result.id == "TRN-005")
        #expect(result.category == .transform)
        #expect(result.outcome == .realized)
        try result.validate()
    }
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func transform006UsesTheProductionLifecycleAndExactSketchCounts() async throws {
        let executor = DefaultCADActivatedCaseExecutor()
        let context = try executor.context(for: "TRN-006")
        #expect(context.challenge.category == .transform)
        #expect(context.challenge.instruction.contains("line"))
        let result = try await executor.evaluate(
            caseID: "TRN-006",
            candidate: CADTransformReferenceCandidate()
        )

        #expect(result.id == "TRN-006")
        #expect(result.category == .transform)
        #expect(result.outcome == .realized)
        try result.validate()
    }
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func transform008UsesTheProductionLifecycleAndExactSketchCounts() async throws {
        let executor = DefaultCADActivatedCaseExecutor()
        let context = try executor.context(for: "TRN-008")
        #expect(context.challenge.category == .transform)
        #expect(context.challenge.instruction.contains("circle"))
        let result = try await executor.evaluate(
            caseID: "TRN-008",
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func transform002CandidateFailureIsTyped() async throws {
        let executor = DefaultCADActivatedCaseExecutor()
        ...
        do {
            ...
            #expect(error == .candidateFailure("TRN-002"))
        }
    }

    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func transform003CandidateFailureIsTyped() async throws {
        let executor = DefaultCADActivatedCaseExecutor()
        ...
        do {
            ...
            #expect(error == .candidateFailure("TRN-003"))
        }
    }

    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func transform004CandidateFailureIsTyped() async throws {
        let executor = DefaultCADActivatedCaseExecutor()
        ...
        do {
            ...
            #expect(error == .candidateFailure("TRN-004"))
        }
    }

    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func transform005CandidateFailureIsTyped() async throws {
        let executor = DefaultCADActivatedCaseExecutor()
        ...
        do {
            ...
            #expect(error == .candidateFailure("TRN-005"))
        }
    }

    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func transform006CandidateFailureIsTyped() async throws {
        let executor = DefaultCADActivatedCaseExecutor()
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func transform008CandidateFailureIsTyped() async throws {
        let executor = DefaultCADActivatedCaseExecutor()
        ...
        do {
            ...
            #expect(error == .candidateFailure("TRN-008"))
        }
    }
