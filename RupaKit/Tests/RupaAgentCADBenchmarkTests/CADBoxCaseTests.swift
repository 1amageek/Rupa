...
struct CADBoxCaseTests {
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func box011CreatesExactMetreCubeThroughProductionController() async throws {
        let result = try await CADBoxCaseRunner(case: .box011).runReference()

        try result.validate()
        #expect(result.caseID == "BOX-011")
        #expect(result.outcome == .realized)
        #expect(result.realized)
        #expect(result.candidateResult?.status == .published)
        #expect(result.candidateResult?.createdFeatureIDs.count == 2)
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func box011OracleRejectsCentimetreScaleSubmissionAfterOnePublicationWithoutRetry() async throws {
        let action = boxAction(
            name: "BOX-011.wrong-unit",
            origin: CADPoint3D(x: -0.25, y: -0.25, z: 0, unit: .centimeter),
            width: CADLength(value: 0.5, unit: .centimeter),
            depth: CADLength(value: 0.5, unit: .centimeter),
            height: CADLength(value: 0.5, unit: .centimeter)
        )
        let result = try await CADBoxCaseRunner(case: .box011).run(action: action)
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func box011RejectsZeroWidthBeforePublication() async throws {
        let action = boxAction(
            name: "BOX-011.zero-width",
            origin: CADPoint3D(x: -0.25, y: -0.25, z: 0, unit: .meter),
            width: CADLength(value: 0, unit: .meter),
            depth: CADLength(value: 0.5, unit: .meter),
            height: CADLength(value: 0.5, unit: .meter)
        )
        let result = try await CADBoxCaseRunner(case: .box011).run(action: action)
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func box011TimeoutIsTypedAndCleansUp() async throws {
        let result = try await CADBoxCaseRunner(
            case: .box011,
            timeoutWallNanoseconds: 1
        ).runReference()
    ...
    @Test
    func box011ReferenceCandidatePreservesPublicMetreCubeValues() async throws {
        let challenge = try CADBenchmarkCatalog().challenge(for: "BOX-011")
        let decision = try await CADBoxReferenceCandidate().decide(
            for: candidateContext(challenge)
        )
        ...
        guard case .action(.automation(.solid(.box(
            ...
            Issue.record("BOX-011 candidate did not produce one solid box action.")
            return
        }
        #expect(name == "BOX-011")
        #expect(origin == CADPoint3D(x: -0.25, y: -0.25, z: 0, unit: .meter))
        #expect(width == CADLength(value: 0.5, unit: .meter))
        #expect(depth == CADLength(value: 0.5, unit: .meter))
        #expect(height == CADLength(value: 0.5, unit: .meter))
    }

    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func box012CreatesExactMillimeterSolidThroughProductionController() async throws {
        let result = try await CADBoxCaseRunner(case: .box012).runReference()

        try result.validate()
        #expect(result.caseID == "BOX-012")
        #expect(result.outcome == .realized)
        #expect(result.realized)
        #expect(result.candidateResult?.status == .published)
        #expect(result.candidateResult?.createdFeatureIDs.count == 2)
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func box012OracleRejectsWrongZPlacementAfterOnePublicationWithoutRetry() async throws {
        let action = boxAction(
            name: "BOX-012.wrong-z",
            origin: CADPoint3D(x: 25, y: 25, z: -50, unit: .millimeter),
            width: CADLength(value: 75, unit: .millimeter),
            depth: CADLength(value: 125, unit: .millimeter),
            height: CADLength(value: 175, unit: .millimeter)
        )
        let result = try await CADBoxCaseRunner(case: .box012).run(action: action)
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func box012RejectsZeroDepthBeforePublication() async throws {
        let action = boxAction(
            name: "BOX-012.zero-depth",
            origin: CADPoint3D(x: 25, y: 25, z: -75, unit: .millimeter),
            width: CADLength(value: 75, unit: .millimeter),
            depth: CADLength(value: 0, unit: .millimeter),
            height: CADLength(value: 175, unit: .millimeter)
        )
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func box012TimeoutIsTypedAndCleansUp() async throws {
        let result = try await CADBoxCaseRunner(
            case: .box012,
            timeoutWallNanoseconds: 1
        ).runReference()
    ...
    @Test
    func box012ReferenceCandidatePreservesPublicMillimeterSolidValues() async throws {
        let challenge = try CADBenchmarkCatalog().challenge(for: "BOX-012")
        let decision = try await CADBoxReferenceCandidate().decide(
            for: candidateContext(challenge)
        )
        ...
        guard case .action(.automation(.solid(.box(
            ...
            Issue.record("BOX-012 candidate did not produce one solid box action.")
            return
        }
        #expect(name == "BOX-012")
        #expect(origin == CADPoint3D(x: 25, y: 25, z: -75, unit: .millimeter))
        #expect(width == CADLength(value: 75, unit: .millimeter))
        #expect(depth == CADLength(value: 125, unit: .millimeter))
        #expect(height == CADLength(value: 175, unit: .millimeter))
    }

    @Test
    func activatedBoxBoundaryContainsOnlyReviewedCases() throws {
        #expect(CADActivatedBoxCase.allCases == [.box001, .box002, .box003, .box004, .box005, .box006, .box007, .box008, .box009, .box010, .box011, .box012])
        for rejectedCaseID in ["CYL-001", "REC-001", "CIR-001"] {
            do {
