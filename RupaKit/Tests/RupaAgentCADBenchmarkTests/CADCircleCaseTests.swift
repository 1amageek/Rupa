...
struct CADCircleCaseTests {
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func executorActivatesCIR001ThroughCIR011AndRejectsCIR012() async throws {
        let executor = DefaultCADActivatedCaseExecutor()
        #expect(executor.activatedCaseIDs.last?.rawValue == "CIR-011")
        ...
        do {
            _ = try executor.context(for: "CIR-012")
            Issue.record("CIR-012 must remain inactive.")
        } catch let error as CADActivatedCaseExecutorError {
            #expect(error == .inactiveCase("CIR-012"))
        }
