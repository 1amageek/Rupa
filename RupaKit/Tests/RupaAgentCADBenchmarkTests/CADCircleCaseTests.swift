...
struct CADCircleCaseTests {
    ...
    @MainActor
    @Test(.timeLimit(.minutes(1)))
    func executorActivatesCIR001ThroughCIR010AndRejectsCIR011() async throws {
        let executor = DefaultCADActivatedCaseExecutor()
        #expect(executor.activatedCaseIDs.last?.rawValue == "CIR-010")
        ...
        do {
            _ = try executor.context(for: "CIR-011")
            Issue.record("CIR-011 must remain inactive.")
        } catch let error as CADActivatedCaseExecutorError {
            #expect(error == .inactiveCase("CIR-011"))
        }
