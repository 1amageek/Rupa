...
@Test func agentReturnsDesignDisplaySnapshotForViewportPlanning() async throws {
    ...
    #expect(snapshot.viewportGridScale.snapStep.meters == session.workspaceState.ruler.minorTickMeters)
    #expect(snapshot.viewportGridScale.configuredMajorStep.meters == session.workspaceState.ruler.majorTickMeters)
    #expect(snapshot.viewportGridScale.workspaceSpan.meters == session.workspaceState.ruler.visibleSpanMeters)
