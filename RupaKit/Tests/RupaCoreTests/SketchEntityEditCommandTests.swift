...
@Test func offsetCurveCommandCreatesParallelSourceLineWithoutChangingOriginal() async throws {
    ...
    #expect(result.didMutate)
    #expect(result.primaryFeatureID == result.createdFeatureIDs.first)
    #expect(result.createdFeatureIDs.count == 1)
    #expect(result.createdFeatureIDs.first == session.document.cadDocument.designGraph.order.last)
    #expect(session.generation == DocumentGeneration(2))
...
@Test func offsetCurveDispatchesGeneratedFaceToFaceLoopOffsetFeature() async throws {
    ...
    #expect(faceLoopOffset.face == (try componentID.stableTopologyReference(
        operationName: "Offset Face Loop test"
    )))
...
@Test func offsetCurveDispatchesGeneratedEdgeWithSupportFaceToEdgeOffsetFeature() async throws {
    ...
    #expect(topology.entries.contains { $0.stableReference == edgeEntry.stableReference })
    #expect(offsetSceneNode.object?.sourceSection == nil)
    #expect(offsetSceneNode.object?.typeID == nil)
    #expect(session.evaluationStatus == .valid)
}
...
@MainActor
@Test func offsetCurveRejectsUnsupportedGeneratedTopologyGapFillBeforeMutation() async throws {
    let session = EditorSession()
    _ = try session.execute(
        .createExtrudedRectangleFromCorners(
            name: "Unsupported Gap Fill Box",
            plane: .xy,
            firstCorner: SketchPoint(
                x: .length(0.0, .millimeter),
                y: .length(0.0, .millimeter)
    ...
    let bodyNodeID = try #require(bodySceneNodeID(for: bodyFeatureID, in: session.document))
    let topology = try TopologySnapshotService().snapshot(document: session.document)
    let supportFaceEntry = try #require(topology.entries.first {
        $0.kind == .face &&
            $0.sceneNodeID == bodyNodeID.description &&
            $0.generatedRole == "startFace"
    })
    ...
    for command in [
        EditorCommand.offsetCurve(
            target: supportFaceTarget,
            distance: .length(2.0, .millimeter),
        ...
        do {
            ...
            Issue.record("Generated topology offsets must reject unsupported gap-fill strategies.")
        } catch let error as EditorError {
            #expect(error.code == .commandInvalid)
            #expect(error.message.contains("non-default gap-fill"))
        }
    }
...
@Test func offsetCurveCommandCreatesSymmetricLineOffsets() async throws {
    ...
    #expect(result.didMutate)
    #expect(result.primaryFeatureID == result.createdFeatureIDs.first)
    #expect(result.createdFeatureIDs.count == 2)
    #expect(result.createdFeatureIDs == Array(session.document.cadDocument.designGraph.order.suffix(2)))
    #expect(session.generation == DocumentGeneration(2))
...
@MainActor
@Test func cutSketchCurveSplitsTargetLineAtSplineCutterIntersection() async throws {
    let session = EditorSession()
    _ = try session.execute(
        .createLineSketch(
            name: "Cut Line Spline Target",
            plane: .xy,
            start: SketchPoint(
                x: .length(0.0, .millimeter),
                y: .length(0.0, .millimeter)
...
@MainActor
@Test func cutSketchCurveSplitsTargetSplineAtSplineCutterIntersection() async throws {
    let session = EditorSession()
    _ = try session.execute(
        .createSplineSketch(
            name: "Cut Spline Spline Target",
            plane: .xy,
            spline: SketchSpline(controlPoints: [
                SketchPoint(x: .length(0.0, .millimeter), y: .length(0.0, .millimeter)),
                SketchPoint(x: .length(10.0 / 3.0, .millimeter), y: .length(0.0, .millimeter)),
    ...
    #expect(targetSegments.allSatisfy { $0.entityKind == "spline" })
    #expect(cutterSegments.count == 1)
    #expect(targetSegments.contains { splineSegmentMatches($0, startX: 0.0, startY: 0.0, endX: 0.005, endY: 0.0) })
    #expect(targetSegments.contains { splineSegmentMatches($0, startX: 0.005, startY: 0.0, endX: 0.010, endY: 0.0) })
    #expect(session.evaluationStatus == .valid)
}
...
@MainActor
@Test func cutSketchCurveSplitsTargetArcAtSplineCutterIntersection() async throws {
    let session = EditorSession()
    _ = try session.execute(
        .createArcSketch(
            name: "Cut Arc Spline Target",
            plane: .xy,
            center: SketchPoint(
                x: .length(0.0, .millimeter),
                y: .length(0.0, .millimeter)
    ...
    _ = try session.execute(
        .createSplineSketch(
            name: "Cut Arc Spline Cutter",
            plane: .xy,
            spline: SketchSpline(controlPoints: [
                SketchPoint(x: .length(0.0, .millimeter), y: .length(-6.0, .millimeter)),
                SketchPoint(x: .length(0.0, .millimeter), y: .length(-2.0, .millimeter)),
                SketchPoint(x: .length(0.0, .millimeter), y: .length(2.0, .millimeter)),
                SketchPoint(x: .length(0.0, .millimeter), y: .length(6.0, .millimeter)),
            ])
        )
...
@MainActor
@Test func cutSketchCurveSplitsTargetCircleAtSplineCutterIntersections() async throws {
    let session = EditorSession()
    _ = try session.execute(
        .createCircleSketch(
            name: "Cut Circle Spline Target",
            plane: .xy,
            center: SketchPoint(
                x: .length(0.0, .millimeter),
                y: .length(0.0, .millimeter)
