...
@Test func editorSessionSketchInputStateTogglesAndClearsOutsideSketchTools() {
    ...
    #expect(session.setSketchDimensionInputAngle(0.5))
    #expect(session.sketchInputState.dimensionInputFocus == .angle)
    ...
    #expect(!session.setSketchDimensionInputAngle(Double.nan))
