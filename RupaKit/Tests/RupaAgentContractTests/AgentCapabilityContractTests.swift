...
@Test func agentCapabilitiesExposeAutomationCommands() async throws {
    ...
    #expect(capabilities.contains("viewportSnapshot"))
...
@Test func agentCapabilityDescriptorsExposeDiscoveryAndMutationContracts() async throws {
    ...
    let sweepPlanGuideAxis = try #require(
        sweepEvaluationPlan.optionMatrix.first { $0.name == "guideStrategyCandidates" }
    )
    ...
    #expect(curveJoin.summary.contains("line/arc/spline endpoints"))
    #expect(curveJoin.summary.contains("G0"))
    #expect(curveJoin.summary.contains("G1"))
    #expect(curveJoin.summary.contains("G2"))
    #expect(curveJoin.summary.contains("curve-analysis readback"))
    #expect(curveJoin.summary.contains("Unjoin"))
    #expect(curveJoin.failureMode.contains("G2 pairs that are not two spline endpoints"))
    #expect(curveJoin.failureMode.contains("line-arc G1 pairs that are not already tangent"))
    ...
    #expect(curveUnjoin.summary.contains("line/arc/spline group"))
    ...
    #expect(designDisplaySnapshot.summary.contains("interaction scale defaults"))
