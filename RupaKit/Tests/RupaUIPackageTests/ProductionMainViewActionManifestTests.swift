...
func productionMainViewActionManifestCountsAreFixed() {
    let rows = ProductionMainViewActionManifest.rows
    #expect(ProductionMainViewActionManifest.sourceCommandRows.count == 85)
...
func productionMainViewGraphAuditIncludesEveryNonLegacyUIAndPresentationSource() throws {
    ...
    #expect(audited.contains("Sources/RupaRendering/MeshSourcePresentationRenderPlan.swift"))
    #expect(audited.contains("Sources/RupaRendering/MeshSourcePresentationSectionGeometryResolver.swift"))
...
func productionMainViewActionManifestSourceCommandSetIsUniqueAndFixed() {
    ...
    #expect(names.count == 85)
    #expect(Set(names).count == names.count)
    #expect(Set(ProductionMainViewActionManifest.sourceCommandRows.map(\.actionID)).count == 85)
}
