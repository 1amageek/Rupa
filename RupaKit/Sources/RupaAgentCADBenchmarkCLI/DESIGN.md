...
`request` validates that the ID is in the activated forty-one-case set and emits
...
- `request` emits valid v1 JSON for an activated line, rectangle, REC-009
  ...
  millimetre/XY case, the complete CIR-001...012 category, and ANG-001...005,
  and rejects inactive `ANG-006`;
