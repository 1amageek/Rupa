...
```text
mounted pointer measurement -> immutable resize baseline -> world preview
                                      -> size + compensating placement intent
                                      -> RupaUI atomic source transaction
```

`ViewportBodyResizeDragTarget` carries the immutable CAD revisions and placement
baseline. RupaUI validates both, then uses the same Core dimension command as
the Inspector with a translation compensating the source extrusion's fixed
profile plane. It never folds this size edit into Transform Scale. Shared source
dimensions retain the Inspector's source-wide semantics. Release uses the same
publication handoff as placement edits. Behavioral evidence must compare the
preview with evaluated committed geometry, fixed opposite corners, unchanged
linear placement, collapse/stale refusal, and single-entry Undo/Redo.
...
3. Entity names, hierarchy, UUIDs, `MeshResource` identity, and collision
   ...
   `centerScale`, `rotate`, and the admitted source-box face/corner resize actions. The
   ...
   actions ends the route with no body transform hit instead of consulting the
   ...
   Both placement branches persist their mutations
   ...
   record needs, and it is the only thing the viewport tells the owner about a
   refusal: this module still decides what counts as reportable, and a
   transient `frameNotReady` is filtered before the funnel rather than by the
   ...
   the refusal funnel and clears the pending interaction, because a drag that
   cannot measure must not leave a handle following the pointer against a
   baseline nothing answered. Clearing the pending interaction is also what
   bounds the report: the next pointer move finds no claim and no drag in
   flight, so it leaves this route rather than refusing again, and one drag
   therefore reports at most once however many moves follow.
...
7. Live pointer, hover, measure endpoint, and canvas-plane resolution consume
   ...
   Every route that resolves snaps against the published document carries that
   pair, not only the hover readout. The creation-drag preview carries it in
   the semantic snapshot it publishes, so the producer resolves the drag's
   start and its end off the main actor against it, and the native world-point
   route carries it into the construction-plane handle drag it resolves on the
   main actor once per pointer move. A route holding no published context for
   the document it passes, such as one reading a projected preview document,
   states none and is answered by the same exact evaluation as before. Each
   resolver on these routes takes the collaborator it calls in its
   initializer, so a test can substitute one whose exact evaluator refuses to
   run and read the forwarding as the difference between that refusal and an
   answer.
...
9. Camera navigation is routed by which pointer button is held, and that
   routing is a contract of `ViewportInputSurface` rather than of the camera
   owner: the primary button belongs to selection and creation gestures, the
   secondary button pans, and the middle button orbits. The middle button is
   the only pointer path to orbit on a mouse; the three-finger indirect touch
   path publishes the same orbit callback, and neither orbit path may pan.

   | Pointer input | Camera operation | Callback |
   |---|---|---|
   | Primary drag | none | selection and creation gestures |
   | Secondary drag | pan | `onPan` |
   | Middle drag | orbit | `onOrbit` |
   | Three-finger indirect touch | orbit | `onOrbit` |

   Pan and orbit share one incremental latch: each pointer step publishes the
   delta since the previous step, never the delta since the press, so a held
   button turns or pans at the rate the pointer moves and a button released
   and pressed again restarts from the new position. A drag whose press
   lands inside an input exclusion rect publishes neither.

10. Hovering the canvas takes the keyboard, so that the viewport's own key
    handling — Escape unwinding, the selection scope keys, and the
    construction-plane key — reaches the tool under the pointer without a
    click first. That transfer is refused while the window is editing text: a
    first responder that is an `NSTextInputClient`, or an `NSControl` holding
    a live field editor, keeps the keyboard. Moving the pointer across the
    canvas may not silently end a rename in the product hierarchy or a
    numeric edit in the inspector, while a click still takes focus
...
14. The prepared graph distinguishes static world resources from bounded
    ...
    | Axis centre-scale marker | 95 pt | 10 pt | Direction-relative marker offset toward the axis tip |
    | Translate cone (arrow tip) | 132 pt | 10 pt | Direction-relative marker offset toward the axis tip |
...
Native test hosts attach production views to never-ordered NSWindows and force
layout. They retain real RealityKit preparation, camera queries and GPU renders;
hidden mounting must not be substituted with a mock camera or CPU success value.
Object-affordance routing tests resolve the receiver from the hosted hierarchy before
delivering an in-process NSEvent. No application activation, global event posting
or window ordering is part of automated verification. Hidden-state assertions
and the runner's static guard reject accidental reintroduction. OS-level focus
and visible compositing remain manual acceptance, not a hidden-host guarantee.
