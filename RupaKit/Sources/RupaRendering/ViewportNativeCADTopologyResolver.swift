...
enum ViewportNativeCADTopologyResolver {
    ...
    ///
    /// No production path reaches this rule. `resolveRegion(in:...)` below
    /// answers the sub-shape rectangle from the region raster, so the visible
    /// sliver this rule may leave unsampled is selected there and lost here.
    /// What is left reachable is this rule's own tests, and RK-4.3.5.6 removes
    /// both once the region path's replacement evidence passes on the mounted
    /// path.
    @available(
        *, deprecated,
        message: "resolveRegion(in:...) answers the sub-shape rectangle from the region raster. Removed in RK-4.3.5.6."
    )
    static func resolve(
        in rect: CGRect,
        topology: ViewportBodyTopology,
