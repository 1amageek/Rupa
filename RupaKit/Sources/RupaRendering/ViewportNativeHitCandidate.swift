...
/// edge, an edge over a face, a face over a sketch region, and a region over
/// the occurrence carrying it, so a pointer that named a sub-shape never
/// resolves to its whole occurrence. `metric` orders candidates of the same
/// rank: projected point distance for a vertex, an edge or a region, and the
/// camera depth of the drawn surface for a face. The caller compares
...
/// happens to answer. Only the one body the native frame draws at the pointer
/// is given a surface hit, so at most one face candidate exists per query and
/// the face metric never has to order two bodies against each other.
///
/// A sketch region is a rank of its own rather than a second family at face
/// rank, because its metric is a projected distance where a CAD face's is a
/// depth. Separating the ranks is what keeps the two from ever being compared
/// by metric: a CAD face drawn at the pointer wins the region beneath it
/// before either metric is read.
///
