...
10. The mounted frame answers a region, not only a point. `Scene.raycast`
    ...
    offscreen kernel numbers on synthetic geometry, and the replacement
    verification below still owes the same measurement on the real mounted
    path. The sampling path was removed before that measurement was taken,
    on the evidence that no production caller reached it, so these numbers
    describe the only rectangle path there is.
