...
public struct Viewport: View {
    ...
    private func viewportHitCandidate(
        ...
        case .sketch(let primitives):
            if let sketchHit = hitScoreForSketch(
                primitives,
                featureID: item.featureID,
                point: point,
                layout: layout
        ...
        case .body(let component):
            if let topologyHit = ViewportBodyTopologyHitTester(tolerance: 8.0).hitTest(
                component: component,
                point: point,
                layout: layout
            ) {
            ...
            if let edge = hitBodyEdge(for: item, point: point, layout: layout) {
                return (
                    ViewportHit(featureID: item.featureID, kind: item.kind.selectableKind, bodyEdge: edge.edge),
                    edge.score
                )
            }
            if let face = hitBodyFace(for: item, point: point, layout: layout) {
                return (
    ...
    private func hitScoreForSketch(
        ...
        point: CGPoint,
        layout: ViewportLayout
    ) -> (entityID: SketchEntityID, controlPointIndex: Int?, score: CGFloat)? {
        var bestHit: (entityID: SketchEntityID, controlPointIndex: Int?, score: CGFloat)?
        for primitive in primitives {
            ...
            case .circle(_, let center, let radiusMeters):
                ...
                controlPointIndex = nil
            case .arc(_, let center, let radiusMeters, let startAngle, let endAngle):
                distance = distanceToProjectedArc(
                    center: center,
                    radiusMeters: radiusMeters,
                    startAngleRadians: startAngle,
                    endAngleRadians: endAngle,
                ...
                controlPointIndex = nil
            case .spline(let entityID, let points, let controlPoints, _):
                let curveDistance = distanceToProjectedPolyline(
                    points: points,
                    point: point,
                    layout: layout
                )
                let controlPointHit = allowsPointHandleInteraction(
                    featureID: featureID,
                    entityID: entityID
                ) ? nearestProjectedControlPoint(
                    controlPoints,
                    point: point,
                    layout: layout
                ) : nil
            ...
            if let current = bestHit {
                if distance < current.score {
                    bestHit = (primitive.entityID, controlPointIndex, distance)
                }
            } else {
                bestHit = (primitive.entityID, controlPointIndex, distance)
            }
        ...
        guard let bestHit, bestHit.score <= 8.0 else {
            return nil
        }
        return bestHit
    }
    ...
    private func contains(
        ...
        if polygonBounds(polygon).contains(point) == false {
            return false
        }
    ...
    private func selectionHits(
        ...
        for item in scene.items {
            ...
            case .sketch(let primitives):
                for primitive in primitives {
                    let bounds = sketchSelectionBounds([primitive], layout: layout).insetBy(dx: -8.0, dy: -8.0)
                    guard !bounds.isNull,
                    ...
                    hits.append(
                        contentsOf: sketchControlPointSelectionHits(
                            primitive,
                            featureID: item.featureID,
                            in: rect,
                            layout: layout
                ...
                for region in item.sketchRegions {
                    let projectedPoints = region.points.map(layout.project)
                    let bounds = polygonBounds(projectedPoints).insetBy(dx: -2.0, dy: -2.0)
                    guard !bounds.isNull,
                          rect.intersects(bounds) else {
                        continue
                    }
            ...
            case .body:
                guard let selectableBounds = selectionBounds(for: item, layout: layout),
                      ...
                      seenBodyFeatureIDs.insert(item.featureID).inserted else {
                    continue
                }
    ...
    private func sketchControlPointSelectionHits(
        ...
        guard case .spline(let entityID, _, let controlPoints, _) = primitive,
              allowsPointHandleInteraction(featureID: featureID, entityID: entityID) else {
            return []
        }
        var hits: [ViewportHit] = []
        for (index, controlPoint) in controlPoints.enumerated() {
            let bounds = pointRect(layout.project(controlPoint), radius: 6.0)
