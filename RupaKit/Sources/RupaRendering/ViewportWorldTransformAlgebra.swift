...
/// Throwing 4x4 world algebra owned by the sketch transform lifecycle.
///
...
/// singular basis with `nil`. A sketch mutation composed through either would
...
enum ViewportWorldTransformAlgebra {
    ...
    /// back into the sketch's local frame, so the mutation is refused rather
    ...
    static func inverted(_ value: Transform3D) throws -> Transform3D {
        ...
        guard determinant.isFinite, abs(determinant) > singularDeterminantFloor else {
            throw RealityViewportSpatialBatch.invalid(
                "A sketch transform frame is singular and cannot be inverted."
            )
        }
        for index in 0 ..< 16 {
    ...
    static func translation(_ vector: Vector3D) throws -> Transform3D {
        guard vector.isFinite else {
            throw RealityViewportSpatialBatch.invalid("A sketch translation is not finite.")
        }
    ...
    static func scale(_ factor: Double, about pivot: Point3D) throws -> Transform3D {
        guard factor.isFinite, pivot.isFinite else {
            throw RealityViewportSpatialBatch.invalid("A sketch scale is not finite.")
        }
    ...
    static func rotation(axis: Vector3D, radians: Double, about pivot: Point3D) throws -> Transform3D {
        guard radians.isFinite, pivot.isFinite, axis.isFinite else {
            throw RealityViewportSpatialBatch.invalid("A sketch rotation is not finite.")
        }
    ...
    static func transformedPoint(_ point: Point3D, by value: Transform3D) throws -> Point3D {
        ...
        guard point.isFinite else {
            throw RealityViewportSpatialBatch.invalid("A sketch transform point is not finite.")
        }
        let w = m[12] * point.x + m[13] * point.y + m[14] * point.z + m[15]
        guard w.isFinite, abs(w) > singularDeterminantFloor else {
            throw RealityViewportSpatialBatch.invalid("A sketch transform point is not representable.")
        }
        let result = Point3D(
    ...
    static func transformedVector(_ vector: Vector3D, by value: Transform3D) throws -> Vector3D {
        let m = try elements(of: value)
        guard vector.isFinite else {
            throw RealityViewportSpatialBatch.invalid("A sketch transform vector is not finite.")
        }
        let result = Vector3D(
        ...
        guard result.isFinite else {
            throw RealityViewportSpatialBatch.invalid("A sketch transform vector is not representable.")
        }
        return result
    }
    ...
    static func normalized(_ vector: Vector3D, describing subject: String) throws -> Vector3D {
        guard vector.isFinite else {
            throw RealityViewportSpatialBatch.invalid("A sketch transform \(subject) is not finite.")
        }
        let length = vector.length
        guard length.isFinite, length > 1.0e-10 else {
            throw RealityViewportSpatialBatch.invalid("A sketch transform \(subject) is degenerate.")
        }
    ...
    private static func elements(of value: Transform3D) throws -> [Double] {
        ...
        for element in values where !element.isFinite {
            throw RealityViewportSpatialBatch.invalid("A sketch transform frame is not finite.")
        }
        return values
    }
    ...
    private static func transform(_ values: [Double], describing subject: String) throws -> Transform3D {
        ...
        do {
            ...
            throw RealityViewportSpatialBatch.invalid("A sketch transform \(subject) is not representable.")
        }
    }
    ...
    private static func minorDeterminant(
        ...
        guard minor.count == 9 else {
            throw RealityViewportSpatialBatch.invalid("A sketch transform minor is malformed.")
        }
