...
public final class ViewportSurfaceRenderer: Sendable {
    ...
    private final class ImmutableGeometry: @unchecked Sendable {
        ...
        init(positions: any MTLBuffer, indices: any MTLBuffer) {
    ...
    public init(plan: MeshSourcePresentationRenderPlan) throws {
        ...
        guard vertexBytes <= plan.retainedByteCount,
              indexBytes <= plan.retainedByteCount - vertexBytes else {
        ...
        try Task.checkCancellation()
        geometry = ImmutableGeometry(positions: positions, indices: indices)
        self.draws = draws
        self.radius = max(radius * 4, 1.0e-9)
        allocatedGeometryByteCount = positions.allocatedSize + indices.allocatedSize
    }
    ...
    private static let shader = """
        ...
        struct Uniforms { float4 x, y, depth, view, light, clip, color; };
        ...
        vertex SurfaceVertex surfaceVertex(uint id [[vertex_id]],
            ...
            return { float4(dot(p, u.x), dot(p, u.y), dot(p, u.depth), 1), p.xyz };
        }
        fragment float4 surfaceFragment(SurfaceVertex p [[stage_in]], constant Uniforms& u [[buffer(1)]]) {
