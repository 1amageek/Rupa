...
public struct RupaCLICommand: ParsableCommand {
...
public struct Capabilities: ParsableCommand {
    ...
    public func run() throws {
        print(RupaCLIService().capabilities().joined(separator: "\n"))
    }
}

extension RupaCLIEditMode: ExpressibleByArgument {}
extension RupaExportPreset.DestinationPolicy: ExpressibleByArgument {}

public enum RupaCLIParameterKind: String, CaseIterable, ExpressibleByArgument, Sendable {
    case length
    case angle
    case scalar
...
public enum RupaCLISketchPlane: String, CaseIterable, ExpressibleByArgument, Sendable {
    case xy
    case yz
    case zx
...
public enum RupaCLIExtrudeDirection: String, CaseIterable, ExpressibleByArgument, Sendable {
    case normal
    case symmetric
...
public struct AgentStatusCommand: ParsableCommand {
    ...
    public var socket: String = RupaAgentSocketPath.defaultPath
    ...
    public func run() throws {
        try RupaCLIExitCode.run {
            let response = try RupaCLIService().agentStatus(
                client: RupaAgentClient(socketPath: RupaAgentSocketPath(socket))
            )
            try RupaCLIOutput.write(response: response, asJSON: json)
        }
    }
}
...
public struct LineSketchCommand: ParsableCommand {
    ...
    public var plane: RupaCLISketchPlane = .xy

    @Option(help: "Edit mode: auto, file, or live.")
    public var mode: RupaCLIEditMode = .auto
    ...
    private func makeAgentClient(
        ...
        if let socket {
            ...
            resolvedSocket = RupaAgentSocketPath.defaultPath
        } else {
            resolvedSocket = nil
        }
        return resolvedSocket.map { socket in
                RupaAgentClient(socketPath: RupaAgentSocketPath(socket))
        }
    }
}
...
public struct BoxModelCommand: ParsableCommand {
    ...
    public var direction: RupaCLIExtrudeDirection = .normal
...
public struct SetParameterCommand: ParsableCommand {
    ...
    public var kind: RupaCLIParameterKind = .length
    ...
    public func run() throws {
        ...
        try RupaCLIExitCode.run {
            ...
            let target = RupaCLIDocumentTarget(
                fileURL: url,
                sessionID: id
            )
            let service = RupaCLIService()
    ...
    private enum ParameterInput {
        ...
        case formula(String, QuantityKind, RupaParameterExpressionDefaults)
    }
    ...
    private func expressionDefaults() throws -> RupaParameterExpressionDefaults {
        switch kind {
        case .length:
            let unitName = unit ?? LengthDisplayUnit.meter.rawValue
            guard let lengthUnit = LengthDisplayUnit(rawValue: unitName) else {
            ...
            return RupaParameterExpressionDefaults(
...
public struct Sessions: ParsableCommand {
    ...
    public func run() throws {
        try RupaCLIExitCode.run {
            let response = try RupaCLIService().sessions(
                client: RupaAgentClient(socketPath: RupaAgentSocketPath(socket))
            )
            try RupaCLIOutput.write(response: response, asJSON: json)
        }
...
public struct ValidateDocument: ParsableCommand {
    ...
    public func run() throws {
        try RupaCLIExitCode.run {
            let url = URL(fileURLWithPath: file)
            let response = try RupaCLIService().validateFile(at: url)
            try RupaCLIOutput.write(
                response: response,
                asJSON: json
            )
...
public enum RupaCLIOutput {
    public static func write(response: RupaCLIResponse, asJSON: Bool) throws {
        try write(
            response,
    ...
    public static func write(
        response: RupaCLIAgentStatusResponse,
        asJSON: Bool
    ) throws {
    ...
    public static func write(
        response: RupaCLISessionsResponse,
        asJSON: Bool
    ) throws {
        try write(
            response,
    ...
    public static func write(
        response: RupaCLIAttachResponse,
        asJSON: Bool
    ) throws {
        try write(
            response,
    ...
    public static func write(
        response: RupaCLIParameterListResponse,
        asJSON: Bool
    ) throws {
    ...
    public static func write(
        response: RupaCLIExportResponse,
        asJSON: Bool
    ) throws {
        try write(
            response,
    ...
    public static func write(
        response: RupaCLIEvaluationResponse,
        asJSON: Bool
    ) throws {
        try write(
            response,
    ...
    public static func write(
        response: RupaCLIMeasurementResponse,
        asJSON: Bool
    ) throws {
        try write(
            response,
    ...
    public static func write(
        response: RupaCLIMeshSummaryResponse,
        asJSON: Bool
    ) throws {
        try write(
            response,
