        ...
        entry(
            ...
            evidence: [
                CADInteractionQualityEvidence(
                    ...
                    notes: [
                        "Loft now has a source-owned Swift-CAD IR, same-boundary-count ruled B-spline and smooth cubic section-direction B-spline B-rep evaluator modes, optional guide curve references that lock first and last section seam samples by first-guide endpoint contact when explicit section starts are absent, multi-section multi-guide rail-following intermediate section rings, explicit per-section startSampleIndex seam starts, automatic cyclic section matching for unspecified seams, solid output, open sheet output, closed section-loop sheet output in ruled mode, RupaCore mutation, Automation, Agent command dispatch, CLI model loft surface-mode creation, measurement, viewport scene display, and Inspector operation summaries.",
                    ]
                ),
            ],
            openWork: [
                "Loft viewport seam placement, continuity-driven smooth rail-surface solving, smooth closed section loops, and G0/G1/G2/G3 section-continuity controls.",
