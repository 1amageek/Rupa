            ...
            evidence: [
                CADInteractionQualityEvidence(
                    ...
                    notes: [
                        ...
                        "Pattern Array summary diagnostics mirror source-owned output invariants for missing instances, mismatched transforms, duplicate ownership, root child mapping, independent-copy feature closure checks, and stale independent-copy definition identity.",
                        "Independent-copy Pattern Array regeneration persists the ComponentDefinition identity used to generate cloned outputs, reuses overlapping output scene roots only while that identity remains unchanged, and rebuilds output features when the source definition identity changes.",
                        "Independent-copy per-output source-divergence checks now tokenize FeatureID references through the shared feature remapper before stable hashing instead of performing JSON string replacement.",
