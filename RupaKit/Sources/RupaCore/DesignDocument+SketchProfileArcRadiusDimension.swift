...
extension DesignDocument {
    ...
    private func profileArcRadiusDimensionBlocksConstraint(
        ...
        case .splineEndpointTangent(let constraint):
            return affectedEntityIDs.contains(constraint.splineEndpoint.splineID) ||
                affectedEntityIDs.contains(constraint.line)
        case .tangentSplineEndpoints(let constraint),
             .smoothSplineEndpoints(let constraint):
            return affectedEntityIDs.contains(constraint.first.splineID) ||
                affectedEntityIDs.contains(constraint.second.splineID)
        }
    }
