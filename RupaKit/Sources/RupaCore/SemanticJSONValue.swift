...
public indirect enum SemanticJSONValue: Codable, Hashable, Sendable {
    case object([String: SemanticJSONValue])
    case array([SemanticJSONValue])
    case string(String)
    case number(Double)
    case bool(Bool)
    case null
    ...
    public init(from decoder: Decoder) throws {
        ...
        if let value = try Self.decodeDouble(from: container) {
            guard value.isFinite else {
                throw DecodingError.dataCorruptedError(
                    in: container,
                    debugDescription: "Semantic JSON numbers must be finite."
                )
            }
            self = .number(value)
            return
        ...
        throw DecodingError.dataCorruptedError(
            in: container,
            debugDescription: "Unsupported semantic JSON value."
        )
    }
    ...
    public func validate() throws {
            ...
            for key in object.keys {
                guard !key.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
                    throw DocumentValidationError.invalidProductMetadata(
                        "Semantic JSON object keys must not be empty."
                    )
                }
            }
        ...
        case .number(let value):
            guard value.isFinite else {
                throw DocumentValidationError.invalidProductMetadata(
                    "Semantic JSON numbers must be finite."
                )
            }
    ...
    private static func decodeObject(from decoder: Decoder) throws -> [String: SemanticJSONValue]? {
        do {
            let container = try decoder.container(keyedBy: DynamicCodingKey.self)
            var object: [String: SemanticJSONValue] = [:]
            for key in container.allKeys {
                object[key.stringValue] = try container.decode(SemanticJSONValue.self, forKey: key)
            }
            return object
    ...
    private static func decodeArray(from decoder: Decoder) throws -> [SemanticJSONValue]? {
        do {
            var container = try decoder.unkeyedContainer()
            var values: [SemanticJSONValue] = []
            while !container.isAtEnd {
                values.append(try container.decode(SemanticJSONValue.self))
            }
            return values
        } catch DecodingError.typeMismatch {
            return nil
