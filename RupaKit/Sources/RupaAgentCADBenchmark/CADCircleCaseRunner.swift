...
@MainActor
struct CADCircleCaseRunner {
    private static let operationName = "createCircleSketch"
    private static let defaultTimeoutWallNanoseconds: UInt64 = 10_000_000_000

    private let activatedCase: CADActivatedCircleCase
    ...
    init(
        case activatedCase: CADActivatedCircleCase,
    ...
    func runReference() async throws -> CADCircleCaseResult {
        try await run(candidate: CADCircleReferenceCandidate())
    }
    ...
    private func project(
        ...
        let evidence = CADCircleRouteEvidence(from: record.routeEvidence)
        ...
        do {
            guard case .circle(let expected) = entry.expected else {
                throw CADCircleOracleError.mismatch(
                    "The activated circle has no private circle expectation."
                )
            }
            let observation = try await record.deadline.run {
                try CADCircleOracle.evaluate(
                    expected: expected,
            ...
            return finish(
                result(
                    ...
                    diagnostics: [
                        "\(caseID.rawValue) circle oracle exceeded its shared deadline after publication; no retry was attempted."
                    ]
                ),
                totalStart: totalStart
            )
        } catch let error as CADCircleOracleError {
            ...
            do {
                ...
                return finish(
                    result(
                        ...
                        diagnostics: [
                            "\(caseID.rawValue) circle failure telemetry read failed: \(message(error))"
                        ]
                    ),
                    totalStart: totalStart
                )
            ...
            return finish(
                result(
                    ...
                    roleBindings: bindings,
                    telemetry: failureTelemetry(record, view: finalView, oracleStart: oracleStart),
                    diagnostics: ["\(caseID.rawValue) circle oracle failed: \(message(error))"]
                ),
                totalStart: totalStart
            )
        }
    ...
    private func telemetry(from record: CADCaseLifecycleRecord) -> CADCircleTelemetry {
        CADCircleTelemetry(
    ...
    private func sourceCounts(in view: ProjectViewSnapshot) throws -> (readCount: Int, entityCount: Int) {
    ...
    private func finish(_ result: CADCircleCaseResult, totalStart: UInt64) -> CADCircleCaseResult {
