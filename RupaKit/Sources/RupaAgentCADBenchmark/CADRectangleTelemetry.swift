...
/// Measured durations and operation counts for one rectangle case.
struct CADRectangleTelemetry: Codable, Equatable, Sendable {
    ...
    func validate(caseID: CADBenchmarkCaseID) throws {
        guard actionCount >= 0,
            ...
            throw CADBenchmarkError.invalidInput(
                ...
                reason: "Rectangle telemetry counts and timeout must be valid."
            )
        }
        guard planningWallNanoseconds <= totalWallNanoseconds,
            ...
            throw CADBenchmarkError.invalidInput(
                caseID: caseID.rawValue,
                reason: "Rectangle phase durations cannot exceed total duration."
            )
        }
    }
}
