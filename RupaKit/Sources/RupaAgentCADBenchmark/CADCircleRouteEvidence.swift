...
/// Coordinates and cleanup evidence for one circle production route.
struct CADCircleRouteEvidence: Codable, Equatable, Sendable {
    ...
    func validate(caseID: CADBenchmarkCaseID) throws {
        guard finalDocumentGeneration >= initialDocumentGeneration,
            ...
            throw CADBenchmarkError.invalidInput(
                ...
                reason: "Circle route coordinates cannot move backwards."
            )
        }
        if didPublish {
            guard initialPublicationSequence < UInt64.max,
                ...
                throw CADBenchmarkError.invalidInput(
                    caseID: caseID.rawValue,
                    reason: "A published circle route must advance once without retry."
                )
            }
        } else {
            guard finalDocumentGeneration == initialDocumentGeneration,
                ...
                throw CADBenchmarkError.invalidInput(
                    caseID: caseID.rawValue,
                    reason: "A non-published circle route must preserve its coordinates."
                )
            }
        }
        guard cleanupCompleted,
            ...
            throw CADBenchmarkError.invalidInput(
                caseID: caseID.rawValue,
                reason: "Circle cleanup must be measured and leave no registration."
            )
        }
    }
}
