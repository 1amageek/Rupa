...
/// Measured durations and operation counts for one box case.
struct CADBoxTelemetry: Codable, Equatable, Sendable {
    ...
    func validate(caseID: CADBenchmarkCaseID) throws {
        guard actionCount >= 0,
            ...
            throw CADBenchmarkError.invalidInput(
                ...
                reason: "Box telemetry counts and timeout must be valid."
            )
        }
        guard planningWallNanoseconds <= totalWallNanoseconds,
            ...
            throw CADBenchmarkError.invalidInput(
                caseID: caseID.rawValue,
                reason: "Box phase durations cannot exceed total duration."
            )
        }
    }
}
