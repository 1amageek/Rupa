...
/// Coordinates and cleanup evidence for one rectangle production route.
struct CADRectangleRouteEvidence: Codable, Equatable, Sendable {
    ...
    func validate(caseID: CADBenchmarkCaseID) throws {
        guard finalDocumentGeneration >= initialDocumentGeneration,
            ...
            throw CADBenchmarkError.invalidInput(
                ...
                reason: "Rectangle route coordinates cannot move backwards."
            )
        }
        if didPublish {
            guard initialPublicationSequence < UInt64.max,
                ...
                throw CADBenchmarkError.invalidInput(
                    caseID: caseID.rawValue,
                    reason: "A published rectangle route must advance once without retry."
                )
            }
        } else {
            guard finalDocumentGeneration == initialDocumentGeneration,
                ...
                throw CADBenchmarkError.invalidInput(
                    caseID: caseID.rawValue,
                    reason: "A non-published rectangle route must preserve its coordinates."
                )
            }
        }
        guard cleanupCompleted,
            ...
            throw CADBenchmarkError.invalidInput(
                caseID: caseID.rawValue,
                reason: "Rectangle cleanup must be measured and leave no registration."
            )
        }
    }
}
