...
|---|---|---|---|---|
| [RupaKit package design](../../DESIGN.md) | parent package | dependency direction and child ownership | Composes verification tools above public production boundaries. | The benchmark target remains an upper-level consumer; the system reaches it through this package. |
...
### CMP-006 compound activation boundary

CMP-006 is the sixth compound case and advances the internal benchmark
authority from 93 to 94. `CADActivatedCompoundCase` contains CMP-001...CMP-006
in catalog order; CMP-007 remains catalog-only and is rejected before category
dispatch. The candidate returns one public `.compound` action with ordered
`hub`, `arm-a`, and `arm-b` members. The hub is a cylinder of radius 20 mm and
depth 50 mm whose base centre is `(0, 0, 0)` mm and axis is `+Z`. Both arms
are 100 by 10 by 10 mm boxes starting at z = 20 mm: `arm-a` has origin
`(20, -5, 20)` mm and `arm-b` has origin `(-120, -5, 20)` mm. Member role
and order are semantic even though the two arm members use the same primitive
operation.

The compound context derives the stable, de-duplicated required operations
`createExtrudedCircle` and `createExtrudedRectangle`, and reports the aggregate
compound capability only when both are available. The runner validates and
lowers all three public members before dispatching one atomic three-command
batch. A malformed or degenerate arm therefore cannot publish a partial
hub/arm sequence.

The private oracle checks the ordered source and independent evaluated solids:
two immutable snapshot reads—one authored-source and one evaluated-topology—
nine sketch entities, six features, three independent bodies, eighteen faces,
thirty-six edges, and twenty-four vertices, with document generation plus
three, transaction revision plus one, publication sequence plus one, one
evaluation pass, and one history entry. Swapping the two arm payloads while
retaining their roles publishes once and is rejected by the role-sensitive
oracle without retry. A zero dimension on the third arm fails before command
dispatch or publication. The shared lifecycle continues to own stale,
candidate failure, timeout, cancellation, telemetry, and cleanup evidence;
the generic three-command rollback proven by CMP-004 is reused without a
case-specific rollback route.

The focused compound and executor suites replay CMP-001...CMP-006 through the
same production route, prove CMP-006 source/B-Rep and lifecycle evidence, and
assert CMP-007 typed inactivity. Adapter, CLI, and schema remain separately
owned module contracts; their sibling designs record the coordinated external
...
complete CMP-001...CMP-007 catalog order; there is no CMP-008. The sphere
category begins at the next authority boundary, SPH-001, and is activated by
the sphere section below only after the compound category gate.
...
lifecycle evidence, and (at this checkpoint) assert SPH-001 typed inactivity.
Adapter, CLI, and schema remain separately owned module contracts; their sibling
designs record the coordinated external 95-case boundary. Catalog/manifest,
Mesh, renderer, and boolean modeling remain outside this activation.

### SPH-001 sphere capability activation boundary
...
exposes neither `createSphere` nor `createAnalyticSphere`, so the pure
descriptor classifier shared by `CADActivatedCaseContextFactory` and
`CADSphereCapabilityObservation` reports `cad.solid.analytic-sphere@1` as
unavailable. The reference candidate must return the typed
`analyticSphereUnavailable` declaration. The runner then records
`expectedUnsupported` with exactly one capability request, one capability
read, zero actions, commands, source mutations, entities, features, bodies,
and publications; the fresh controller has zero registrations before and
after the observation.

The runner has no project workspace, initial document, CAD operation, kernel
construction, B-Rep observation, or Mesh substitute path. A candidate action
or finish decision is an invalid submission with no mutation. A candidate
timeout or cancellation is terminal and leaves the same zero-mutation
coordinates. An arbitrary candidate error is rethrown to
`DefaultCADActivatedCaseExecutor`, whose wrapper exposes the typed
`candidateFailure` error; it is not converted into a successful or generic
execution outcome. The exact analytic-sphere oracle remains independent and
continues to reject circle, cylinder, polyhedron, Mesh, or any other
non-analytic substitute for later genuine sphere support.

The focused sphere and executor tests prove the SPH-001 route, the
SPH-002...SPH-005 activation boundary, value-equal request/live capability
contexts, typed unsupported outcome, candidate failure propagation, timeout,
cancellation, substitute rejection, zero publication, and cleanup. JSON and
CLI remain separately owned: their production source, schema, codec, and exit
mapping are unchanged, while their sibling designs synchronize the external
...
At this boundary, `CADActivatedSphereCase` contains SPH-001 and SPH-002 in
catalog order and SPH-003 remains typed inactive. Case-local tests prove the exact target values,
...
### SPH-003 sphere capability activation boundary

SPH-003 advances the activated authority from 97 to 98 through the unchanged
production Agent capability-observation path. Its public target preserves meter
units: radius 0.1 m and center `(0, 0, 0.1)` m. These values describe a
...
depth-0.2 m cylinder before publication. The independent oracle rejects an
analytic sphere moved to z 0 m and a non-analytic cylinder even when its other
