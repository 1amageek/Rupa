...
| `CADActivatedCircleCase` | Internal; the reviewed circle IDs that may enter behavioral execution | Began with CIR-001, currently contains CIR-001...007, and advances one reviewed case per later commit |
...
continuation, activate unreviewed IDs such as `CIR-008`, schedule several cases, or establish a
benchmark baseline.
...
`T12-CIR-007` advances the same authority to the exact thirty-one-case prefix
...
keeps CIR-008 typed inactive. No shared circle infrastructure, schema, catalog,
