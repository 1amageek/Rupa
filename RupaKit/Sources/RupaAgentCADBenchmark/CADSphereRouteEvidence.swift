...
/// Coordinates and cleanup evidence for one analytic-sphere production route.
    ...
    func validate(caseID: CADBenchmarkCaseID) throws {
        guard finalDocumentGeneration >= initialDocumentGeneration,
            ...
            throw CADBenchmarkError.invalidInput(
                ...
                reason: "Sphere route coordinates cannot move backwards."
            )
        }
        if didPublish {
            guard initialPublicationSequence < UInt64.max,
                ...
                throw CADBenchmarkError.invalidInput(
                    caseID: caseID.rawValue,
                    reason: "A published sphere route must advance once without retry."
                )
            }
        } else {
            guard finalDocumentGeneration == initialDocumentGeneration,
                ...
                throw CADBenchmarkError.invalidInput(
                    caseID: caseID.rawValue,
                    reason: "A non-published sphere route must preserve its coordinates."
                )
            }
        }
        guard cleanupCompleted,
            ...
            throw CADBenchmarkError.invalidInput(
                caseID: caseID.rawValue,
                reason: "Sphere cleanup must be measured and leave no registration."
            )
        }
    }
}
