...
/// Coordinates and cleanup evidence for one box production route.
struct CADBoxRouteEvidence: Codable, Equatable, Sendable {
    ...
    func validate(caseID: CADBenchmarkCaseID) throws {
        guard finalDocumentGeneration >= initialDocumentGeneration,
            ...
            throw CADBenchmarkError.invalidInput(
                ...
                reason: "Box route coordinates cannot move backwards."
            )
        }
        if didPublish {
            guard initialPublicationSequence < UInt64.max,
                ...
                throw CADBenchmarkError.invalidInput(
                    caseID: caseID.rawValue,
                    reason: "A published box route must advance once without retry."
                )
            }
        } else {
            guard finalDocumentGeneration == initialDocumentGeneration,
                ...
                throw CADBenchmarkError.invalidInput(
                    caseID: caseID.rawValue,
                    reason: "A non-published box route must preserve its coordinates."
                )
            }
        }
        guard cleanupCompleted,
            ...
            throw CADBenchmarkError.invalidInput(
                caseID: caseID.rawValue,
                reason: "Box cleanup must be measured and leave no registration."
            )
        }
    }
}
