...
@MainActor
public struct RupaMainView: View {
    ...
    private let agentHost: RupaAgentHost?
    ...
    private var selectedSceneNodeIDsBinding: Binding<Set<RupaSceneNodeID>> {
        Binding(
            get: {
    ...
    private var sceneBrowserRows: [SceneBrowserRow] {
        ...
        func append(_ id: RupaSceneNodeID, depth: Int) {
    ...
    private var componentDefinitionIDs: [RupaComponentDefinitionID] {
        session.document.productMetadata.componentDefinitions.values
            .sorted { $0.name < $1.name }
            .map(\.id)
    }

    private var filteredComponentDefinitionIDs: [RupaComponentDefinitionID] {
        guard !normalizedSidebarSearchText.isEmpty else {
    ...
    private var componentInstanceIDs: [RupaComponentInstanceID] {
        session.document.productMetadata.componentInstances.values
            .sorted { $0.name < $1.name }
            .map(\.id)
    }

    private var filteredComponentInstanceIDs: [RupaComponentInstanceID] {
        guard !normalizedSidebarSearchText.isEmpty else {
    ...
    private func handleViewportPick(_ target: ViewportCanvasTarget) {
        ...
        let targetSceneNodeID: RupaSceneNodeID?
    ...
    private func uniqueSceneNodeIDs(for hits: [ViewportHit]) -> [RupaSceneNodeID] {
        var ids: [RupaSceneNodeID] = []
    ...
    private func sceneNodeID(for hit: ViewportHit) -> RupaSceneNodeID? {
        let expectedKind: RupaSceneNodeReference.Kind = switch hit.kind {
        case .sketch:
            .sketch
        case .body:
            .body
    ...
    private func setHoveredSceneNode(_ id: RupaSceneNodeID?) {
    ...
    private func setHoveredSceneNode(_ id: RupaSceneNodeID, isHovered: Bool) {
        if isHovered {
    ...
    @ViewBuilder
    private func componentBrowserRow(_ id: RupaSceneNodeID, depth: Int) -> some View {
    ...
    @ViewBuilder
    private func componentDefinitionRow(_ id: RupaComponentDefinitionID) -> some View {
    ...
    @ViewBuilder
    private func componentInstanceRow(_ id: RupaComponentInstanceID) -> some View {
    ...
    private func sceneNodeSystemImage(for reference: RupaSceneNodeReference?) -> String {
    ...
    private func toggleSceneNodeVisibility(_ id: RupaSceneNodeID) {
    ...
    private func toggleSceneNodeLock(_ id: RupaSceneNodeID) {
        guard let node = session.document.productMetadata.sceneNodes[id] else {
            return
        }
    ...
    private func toggleComponentInstanceVisibility(_ id: RupaComponentInstanceID) {
        guard let instance = session.document.productMetadata.componentInstances[id] else {
            return
        }
    ...
    private func toggleComponentInstanceLock(_ id: RupaComponentInstanceID) {
        guard let instance = session.document.productMetadata.componentInstances[id] else {
            return
        }
    ...
    private var selectedSceneNodes: [RupaSceneNode] {
    ...
    @ViewBuilder
    private func objectInspectorSections(_ nodes: [RupaSceneNode]) -> some View {
        inspectorSection(nodes.count == 1 ? "Selection" : "Selection Group") {
            if nodes.count == 1, let node = nodes.first {
                inspectorRow("Name", node.name)
                inspectorRow("Kind", sceneNodeKindTitle(for: node.reference))
                ...
                inspectorRow("Kinds", valueSummary(nodes.map { sceneNodeKindTitle(for: $0.reference) }))
    ...
    private func boolChoicePicker(
        _ title: String,
        nodes: [RupaSceneNode],
        keyPath: KeyPath<RupaSceneNode, Bool>,
        apply: @escaping (RupaSceneNodeID, Bool) -> Void
    ) -> some View {
    ...
    @ViewBuilder
    private func sourceGeometrySection(_ nodes: [RupaSceneNode]) -> some View {
        let extrudeFeatureIDs = nodes.compactMap { extrudeFeatureID(for: $0) }
        if !extrudeFeatureIDs.isEmpty {
            inspectorSection("Size") {
                transformLengthControl(
                    "Depth",
                    values: extrudeFeatureIDs.compactMap { resolvedExtrudeDistance(featureID: $0) },
                    sliderRange: sizeSliderRange
                ) { meters in
                    for featureID in extrudeFeatureIDs {
                        session.setExtrudeDistance(
                            featureID: featureID,
                            distance: .length(max(meters, 0.0), .meter)
                        )
                    }
                }
            }
        ...
        inspectorSection("Corners") {
            inspectorRow("Fillet Radius", "Not Available")
            inspectorRow("Chamfer", "Not Available")
        }
    }
    ...
    private func translation(for node: RupaSceneNode) -> InspectorVector3D {
    ...
    private func scale(for node: RupaSceneNode) -> InspectorVector3D {
        let values = normalizedMatrixValues(node.localTransform.matrix.values)
    ...
    private func extrudeFeatureID(for node: RupaSceneNode) -> FeatureID? {
    ...
    @ViewBuilder
    private func materialPicker(_ nodes: [RupaSceneNode]) -> some View {
    ...
    private func materialChoice(for nodes: [RupaSceneNode]) -> InspectorMaterialChoice {
    ...
    @ViewBuilder
    private func referenceInspectorRows(for reference: RupaSceneNodeReference) -> some View {
        switch reference.kind {
    ...
    private func transformSummary(for nodes: [RupaSceneNode]) -> String {
    ...
    private func referenceTitle(for reference: RupaSceneNodeReference?) -> String {
        guard let reference else {
            return "Group"
        }
    ...
    private func referenceSummary(for nodes: [RupaSceneNode]) -> String {
        valueSummary(nodes.map { referenceTitle(for: $0.reference) })
    }

    private func parentTitle(for id: RupaSceneNodeID) -> String {
    ...
    private func parentSummary(for nodes: [RupaSceneNode]) -> String {
        valueSummary(nodes.map { parentTitle(for: $0.id) })
    }

    private func parentSceneNode(for id: RupaSceneNodeID) -> RupaSceneNode? {
        session.document.productMetadata.sceneNodes.values.first { node in
            node.childIDs.contains(id)
        }
    }

    private func descendantCount(for id: RupaSceneNodeID) -> Int {
        guard let node = session.document.productMetadata.sceneNodes[id] else {
            return 0
        }
    ...
    private func sceneNodeKindTitle(for reference: RupaSceneNodeReference?) -> String {
        guard let reference else {
            return "Group"
        }
        switch reference.kind {
    ...
    private func componentDefinitionName(for id: RupaComponentDefinitionID) -> String {
