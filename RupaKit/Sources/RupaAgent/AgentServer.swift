...
public final class AgentServer: AgentClientProtocol {
    ...
    private static let capabilityCatalog: [AgentCapabilityDescriptor] = [
        ...
        capability(
            ...
            failureMode: "Rejects missing profile, path, guide, or target body features, duplicate references, invalid option quantities, curved-path parallel guide constraints, profile-plane degenerate parallel alignment, oblique parallel alignment with twist, scale, or guides, round corner style, simplify output, stale generations, targetless boolean operations, new-body target references, collapsed section scale, non-contacting point/chord guide starts, curve-guide path/profile contact failures, conflicting signed-axis rail guides, overconstrained guide sets, and degenerate swept topology before committing invalid geometry through the shared typed sweep evaluation contract; evaluates straight-path normal alignment as a path-normal section sweep, straight-path parallel alignment as profile-plane-preserving exact extrusion when the path has a profile-normal component, curved-path parallel alignment as a profile-plane parallel section sweep with twist and scale, straight-path exact swept-sheet side surfaces for identity section transforms without guides, polygonal swept-solid new-body subsets for curved, twisted, scaled, compatible multiple point/chord guide constraints, non-uniform affine and signed-axis point-guide rail deformation, and curve-guide contact constraints, and polygonal swept-sheet subsets for curved, transformed, or guided sheet inputs; evaluates exact box-prism union, difference, intersection, and slice boolean sweeps with target replacement, separated-fragment difference output, z-through rectangular-frame difference output, orthogonal cell-union connected box difference output, or keep-tools generated-name coverage; reports unsupported evaluation for non-box boolean operands, connected boolean topology outside the axis-aligned box cell-union subset, exact swept surfaces outside the straight identity analytic-boundary subset, and incompatible guide constraints."
        ),
        capability(
