...
public final class AgentCommandController: AgentClientProtocol {
    ...
    private static let capabilityCatalog: [AgentCapabilityDescriptor] = [
        ...
        capability(
            ...
            category: .read,
            summary: "Preflight a proposed standalone Boolean without mutating the document, returning the exact operand subset, output topology kind, topology name scheme, B-rep topology counts, primitive counts, unsupported code, and ordered checks used by the shared Boolean evaluation contract.",
            access: .agentRequest,
            mutatesDocument: false,
        ...
        capability(
            ...
            optionMatrix: [
                ...
                AgentCapabilityDescriptor.OptionAxis(
                    ...
                    notes: [
                        "ruled creates degree-1 section-direction side faces and straight connector edges",
                        "smooth creates cubic section-direction side faces and cubic connector edges for open section chains",
                        "smooth supports closed section-loop sheets when resultKind is sheet and closesSectionLoop is true"
                    ]
                ),
                AgentCapabilityDescriptor.OptionAxis(
                    name: "smoothTangentScale",
                    supportedValues: ["positiveFiniteScalar"],
                    notes: [
                ...
                AgentCapabilityDescriptor.OptionAxis(
                    name: "sectionSmoothTangentMode",
                    supportedValues: ["automatic", "zero"],
                    notes: [
                        "automatic uses the smooth Loft finite-difference tangent for the authored section",
                        "zero clamps that authored section's cubic section-direction handle to zero length",
                        "this is a handle boundary condition, not a G-continuity solve against adjacent surfaces"
                    ]
                ),
                AgentCapabilityDescriptor.OptionAxis(
                ...
                AgentCapabilityDescriptor.OptionAxis(
                    ...
                    notes: [
                        ...
                        "the first guide locks section seams before boundary-progress matching when explicit section starts are absent",
                        "one or more open guide curves can add rail-following intermediate section rings between authored Loft sections",
                        "exact smooth rail-surface solving beyond section-ring deformation remains unsupported"
                    ]
                ),
                AgentCapabilityDescriptor.OptionAxis(
