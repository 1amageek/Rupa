...
Canvas overlays are editing affordances, not document headers. They must preserve maximum modeling area and keep viewport input readable.

| Contract | Requirement |
|---|---|
| Height | Top Canvas HUDs share the same compact container height and inner control height through `ViewportCanvasChromeMetrics` and `WorkspaceChromeControlMetrics`. |
| Material | Canvas overlay chrome uses the shared borderless Liquid Glass container. Overlay controls must not introduce a second border style unless the shared metrics change. |
| Footprint | Canvas HUDs are content-sized. They must not use filler spacers or stretch across the viewport when the same information is already available elsewhere. |
| Title ownership | The document title belongs to Navigation chrome. Canvas HUDs must not repeat the file name or document title. |
| Status ownership | Canvas HUDs may show compact viewport-local state such as scale, unit, selection count, and immediate actions; document-level status belongs in Navigation, Inspector, Logs, or diagnostics surfaces. |
...
| Bridge Curves | Select two curve or surface boundary targets; expose continuity level, tangent direction, and tangent magnitude controls; produce an editable source curve rather than a transient display curve. Viewport endpoint handles must render tangent guides with viewport-readable projected length instead of fixed model-space distances, so they remain usable from mechanical work to site-scale layouts. |
...
| `ProjectPackageStore` | Owns schema-v3 `.rupa` package persistence for Product, optional CAD, and optional Authored Mesh sources. Legacy native archives are rejected at the project-access boundary. |
...
| `setFeatureSuppression` | Sets one existing Swift-CAD feature's suppressed state as an undoable source mutation. The command rejects missing features and rejects suppression states that would leave an active feature depending on a suppressed source before mutation. Reapplying the current state is a no-op and does not advance generation or dirty the document. |
...
flowchart LR
    ...
    Evaluate --> Names["subshapes: SubshapeIndex"]
...
| Precision | Results include workspace precision reports when evaluated geometry is too far from the origin for the current tolerance budget. Reports carry the reason, measured coordinate resolution, model span, and recommended rebase translation so UI, Agent, and CLI workflows can move source geometry back to a local origin through `rebaseWorkspaceOrigin`. |
...
| Inspector / Precision | Canvas precision context | Workspace precision reason, origin distance, model span, and recommended rebase translation when the evaluated model is outside the current precision budget | Apply the recommended translation through `rebaseWorkspaceOrigin`; hide the section when no actionable rebase translation exists |
...
Live mode routes the command to the running app through the same
`ProjectAccessSession` used by every project command.
...
|---|---|
| Session resolution | Resolve by canonical `.rupa` project URL or explicit session ID through the access opener. |
...
| Dirty state | Successful unsaved mutations mark the document dirty; only the explicit save command persists them. |
...
| `snap.resolve` | Resolve a point against shared snapping rules and return ranked snap diagnostics; omitted grid intervals use the document ruler minor tick, while explicit intervals remain fixed. |
...
flowchart LR
    ...
    Resolver --> Persistent["Swift-CAD SubshapeID + StableSubshapeReference"]
...
| `rupa view list <document>` | List saved views with camera, projection, section, visibility, and display-scale metadata. |
...
| `rupa view projection <document> --id <uuid> [--tolerance-meters <value>] [--maximum-stroke-count <count>] [--svg-output <path>] [--pdf-output <path>] [--drawing-page <preset>] [--drawing-style <preset>]` | Generate structured orthographic drawing projection strokes and visible/hidden visibility segments from a saved view without mutating the document, optionally writing deterministic hidden-line SVG and PDF files from the same result with shared Letter/A4 page and technical/presentation style presets. |
...
| `rupa feature unsuppress <document> <feature>` | Unsuppress a feature. |
