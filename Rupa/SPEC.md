...
| `createSlotSketch` | Creates a source-owned Slot profile from a selected open source curve target. The current supported subset accepts a selected source line, connected open source line-chain, open source arc, connected open line/arc chain, or open cubic Bezier spline. Line/arc-chain targets validate positive width, non-zero source segment lengths, open-chain topology, non-branching topology, full-circle arc rejection, non-collapsing inner arc radius, connected offset joins, tangent cap validity, and sampled source/profile self-intersection before mutation. Spline targets are sampled through `SketchCurveSampler` into a centerline profile path, reject closed splines before mutation, and reuse the same Slot profile validation. The command creates a closed profile in the same sketch plane from two symmetric offsets plus two tangent semicircular arc caps, stores Slot object metadata including `source.kind`, width, source path length, cap radius, and profile arc tessellation policy, routes through Core, workspace context controls, Inspector line controls, selected source-line viewport width arrows, Automation, and Agent, and is also reachable through `offsetCurve` Slot mode for the same supported subset. Line-chain, arc, line/arc-chain, and sampled-spline Slot profiles remain extrudable through profile tessellation; points, closed circles, closed or branched line/arc chains, full-circle arcs, closed splines, and unsupported targets return typed diagnostics before mutation. |
| `joinSketchCurves` | Joins two selected source curve targets in the same sketch. The current supported subset accepts two distinct source lines whose endpoint coordinates have exactly one aligned endpoint pair and whose line directions are collinear. The command retains the target line entity ID, removes the adjacent line entity, rewrites the retained line to span the two outer endpoints, removes the joined interior endpoint coincidence, migrates safe adjacent outer endpoint `fixed`, `coincident`, `distance`, and `angle` references onto the retained target endpoint, stores a `JoinedCurveSource` ownership snapshot with the original two source lines plus before/after constraints and dimensions, and routes through Core, Automation, Agent, and selected-line Inspector controls. The Inspector enables Join only when the selection contains another same-sketch source line; Core still performs endpoint and collinearity validation before mutation. The command rejects different sketches, same-curve pairs, non-line targets, missing or ambiguous aligned endpoint pairs, non-collinear lines, dimensions or constraints attached to the disappearing interior endpoint, whole-curve relationships on the removed line, existing joined-curve ownership, generated Bridge Curve metadata dependencies, and stale generations before mutation. Joined-curve groups beyond this two-line ownership snapshot, line/arc/spline joins, continuity controls, downstream profile ownership policy, and viewport endpoint-pair picking remain follow-up work. |
| `unjoinSketchCurve` | Restores a retained source line from the current `joinSketchCurves` subset back into the original two source line entities. The command requires the selected target to match a `JoinedCurveSource` retained entity, verifies the current retained line geometry plus constraints and dimensions still match the stored post-join snapshot, restores the original retained and adjacent line sources, restores the pre-join constraints and dimensions, removes the joined-curve ownership source, and routes through Core, Automation, Agent, and selected-line Inspector controls. The Inspector shows the joined source ID and enables Unjoin only for retained joined source lines. It rejects targets without joined-curve ownership, non-line targets, changed joined-line geometry, changed constraints or dimensions, restored entity ID collisions, generated Bridge Curve metadata dependencies, and stale generations before mutation. |
