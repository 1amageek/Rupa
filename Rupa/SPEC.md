...
| `applySelectionDimensionTarget` | Applies the stored target of one existing persistent Swift-CAD selection dimension to supported source geometry as an undoable source mutation. The current supported subset is a distance dimension whose two references are the current start and end parameters of the same source line curve. The command resolves the curve reference back to the owning sketch entity, routes the edit through `setSketchEntityDimension(.length)`, then rewrites the stored line endpoint parameters to match the updated line length so later evaluation remains stable. Unsupported topology, non-line curves, angle dimensions, stale endpoint parameters, invalid targets, and fixed/constraint conflicts fail before mutation. |
...
| `rupa dimension apply-selection <document> --dimension-id <uuid>` | Apply the stored target of an existing persistent CAD selection dimension to supported source line length or source circle/arc radius targets. |
