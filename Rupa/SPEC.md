...
| `joinSketchCurves` | Joins two selected source curve targets in the same sketch. The current supported subset accepts two distinct source lines whose endpoint coordinates have exactly one aligned endpoint pair and whose line directions are collinear. The command retains the target line entity ID, removes the adjacent line entity, rewrites the retained line to span the two outer endpoints, removes the joined interior endpoint coincidence, migrates safe adjacent outer endpoint `fixed`, `coincident`, `distance`, and `angle` references onto the retained target endpoint, and routes through Core, Automation, and Agent. The command rejects different sketches, same-curve pairs, non-line targets, missing or ambiguous aligned endpoint pairs, non-collinear lines, dimensions or constraints attached to the disappearing interior endpoint, whole-curve relationships on the removed line, generated Bridge Curve metadata dependencies, and stale generations before mutation. Unjoin Curve, joined-curve groups, line/arc/spline joins, continuity controls, downstream profile ownership policy, and viewport endpoint-pair picking remain follow-up work. |
