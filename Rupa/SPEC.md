...
Canvas overlay chrome is intentionally compact because it covers the modeling surface. Top canvas controls use the shared `ViewportCanvasChromeMetrics` height, content height, padding, corner radius, and outline policy. Viewport badges, top command controls, and bottom context controls use the same Liquid Glass container treatment, stay content-sized, and do not include document filenames because the active document title belongs to the navigation title.
