...
| Alternative Duplicate | Project selected source sketch curves onto the supplied or active construction plane along that plane normal and create a new editable source curve sketch. The current contract is `projectSketchCurvesToConstructionPlane`: exact projection for lines and cubic Bezier splines on any valid target plane, exact circle and arc projection only when source and target planes are parallel, active construction-plane fallback when no plane is supplied, result geometry role `.curve`, and Core/Inspector/Automation/Agent parity. The command rejects source points, duplicate targets, collapsed projected lines, nonparallel circular projections until exact ellipse/conic sketch sources exist, invalid planes, invalid output sketches, and stale generations before mutation. |
