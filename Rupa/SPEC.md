...
| Alternative Duplicate / Duplicate Curve and Project / Project Outline | Project selected source sketch curves, generated edges, or generated body outlines onto the supplied or active construction plane along that plane normal and create a new editable source curve sketch. The current curve/edge contract is `projectSketchCurvesToConstructionPlane`: exact projection for source lines, generated line edges, and cubic Bezier splines on any valid target plane; exact source circle/arc and generated circular-edge projection only when source and target planes are parallel. The current outline contract is `projectBodyOutlinesToConstructionPlane`: selected generated body objects project their generated line/circular edge outlines onto the target plane, deduplicate coincident projected front/back edges, skip depth edges that collapse along the projection normal, and produce a source sketch with geometry role `.curve`. Both commands use active construction-plane fallback when no plane is supplied and expose Core/Inspector/Automation/Agent parity. They reject source points, duplicate targets, unresolved generated edges, non-body outline targets, generated edge kinds other than line/circle, collapsed single-curve projections, empty non-collapsed outlines, nonparallel circular projections until exact ellipse/conic sketch sources exist, invalid planes, invalid output sketches, and stale generations before mutation. |
