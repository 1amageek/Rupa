...
```mermaid
flowchart LR
    Operations["ProjectOperating\nevaluate / commit / history / load / save"] --> State["ProjectStateSnapshot\none publication sequence"]
    State --> Build["ProjectViewSnapshotBuilder\noff MainActor"]
    Build --> View["ProjectViewSnapshot\npresentation + interaction"]
    View --> UI["MainActor UI"]
    State -. "package and mutation authority" .-> Operations
```

`ProjectViewSnapshot` is a read-only UI projection. It contains presentation
geometry, the exact selected representation identity and source reference,
selection, workspace state, history/dirty flags, an optional same-generation CAD
interaction context, and an explicit occurrence-to-scene-node index. It does not
contain package bytes, editable source, or mutation authority. The index is
