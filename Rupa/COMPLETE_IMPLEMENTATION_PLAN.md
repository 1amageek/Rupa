...
| CAD/Mesh responsibility authority | `CAD_MESH_RESPONSIBILITY_CONTRACT.md` |
...
| M3 Agent and CLI parity | Agent and CLI can discover, preflight, execute, dry-run, and query every supported UI operation. | Scalar/choice domain forms and generic registered execution are implemented. Selection-reference/collection/file domain inputs, authenticated live-access safety, rollback evidence, and readback summaries for planning remain. | Agent contract tests, runtime tests, signed CLI live-access tests, and golden JSON response tests. |
...
| Agent/CLI lane | Automation or domain execution contract is fixed. | Discovery, dry-run, authenticated live execution, explicit-save rollback evidence, JSON tests. |
