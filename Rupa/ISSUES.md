...
| C-5 | Loft shell inverts when advancing against the first section's sketch normal | Profiles are CCW-normalized about their sketch normal; lofting downward (advance ≈ −sketchNormal) makes every side and cap normal point inward uniformly. Volume survives (`abs`), but the body renders inside-out and face-orientation consumers get an inverted solid. Fix: derive a ring-winding-vs-advance sign once and set `Face.orientation` (revolve-style). | 2026-07-07 loft audit |
| C-6 | Cut Curve misplaces second and later cuts on multi-segment splines | `DesignDocument+SketchCurveCut.swift:31-56` composes global fractions linearly across successive splits, but spline parameters are uniform-per-Bezier-segment and `splitSpline` renormalizes the remainder, so the 2nd+ cut lands off the cutter silently (e.g. cuts at 0.25/0.75 → second cut at original 0.667). Same linear remap defect in `rewriteBridgeEndpointAfterCurveSplit`. Fix: split from sorted fractions against the original knot structure. | 2026-07-07 sketch-tools audit |
...
| C-9 | Rebuild deviation uses power-basis conversion on absolute coordinates | `cubicBezierPowerCoefficients` differences absolute control points (far-origin class); at ~1e12 sketch coordinates deviation noise reaches ~1e-3 m and refit tolerance loops cannot converge. Fix: rebase control points to a local origin first. | 2026-07-07 sketch-tools audit |
| C-10 | Boolean `rectangularUnion` volume epsilon silently swallows small cuts | `BoxBRepBooleanEvaluator.swift:383` budgets rectangularity with a LINEAR tolerance as an absolute volume (1e-6 m³ = 1 cm³ for bodies ≤ 1 m³): subtracting a 5 mm pocket from a 100x100x10 mm plate returns the uncut plate as a silent no-op. Fix: decide rectangularity combinatorially from the cell grid, or budget volume as tolerance x exposed face area. | 2026-07-07 sweep/boolean audit |
| C-11 | Parallel-alignment curved sweep never checks path advance sign vs profile normal | A profile on XY swept along a path descending in −Z builds every facet and cap inward (inside-out solid); the enclosure gate uses `abs` so it validates silently. Mirror the extrude `extrusionSign` pattern and reject mid-path sign flips. | 2026-07-07 sweep/boolean audit |
...
| R-5 | Unbounded while-loop angle normalization | `resolvedAngleValue` only checks `isFinite`; a huge-but-finite angle expression (1e18 rad) hangs `while span <= 0 { span += 2π }` loops across cut/corner/slot geometry. Fix: `truncatingRemainder` first. | 2026-07-07 sketch-tools audit |
