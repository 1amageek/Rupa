...
not add cases to `EditorCommand` and RupaCore does not decode concrete domain
payloads.
...
| Universal commands | Ordered mutating RupaCore/Automation commands generated by the domain adapter. |
...
Concrete domain modules never add command cases to `EditorCommand` and never pass
...
An external side effect is never hidden inside an `EditorCommand`.
