...
JSON. It is a child of the [RupaBenchmarks package design](../../DESIGN.md), has no
...
|---|---|---|---|---|
| [RupaBenchmarks package](../../DESIGN.md) | parent | executable product and target graph | Registers this isolated native tool. | It must not change Xcode `RupaCLIProduct` or `RupaCLIKit`. |
...
| [RupaAgentCADBenchmark](../../../RupaKit/Sources/RupaAgentCADBenchmark/DESIGN.md) | transitive dependency | activated executor and production/oracle result | Executes exactly one reviewed case. | The CLI cannot activate a catalog-only case. |
...
`request` validates that the ID is in the activated 100-case set and emits
...
The executable follows the benchmark-owned [circle case matrix](../../../RupaKit/Sources/RupaAgentCADBenchmark/DESIGN.md#cir-004-through-cir-012-case-matrix)
...
`(109.53353488403286, -22.41438680420134, 25)` mm, and TRN-005 wrong-order
translation `(15.849364905389024, 50, 77.4519052838329)` mm exit `2` as
`invalidSubmission` after one publication without retry. TRN-006's inverse-axis
substitute has the same postpublication result. TRN-007's 90-degree wrong-angle
substitute also has the same postpublication result. TRN-008's translate-before-
rotate equivalent translation also exits `2` after one publication without
retry; a zero rotation axis
...
appending the actual TRN-005 request freezes the 85-request aggregate as
`c5c61320be710e1fe290b86e25ad9afb83b2d687c608c13aa3b4c6e2ee100779`;
appending the actual TRN-006 request freezes the 86-request aggregate as
`f7061d03903393f8384f3c23eed29c4eb4ef8529e27f03fca5e268e698b01f90`;
appending the actual TRN-007 request freezes the 87-request aggregate as
...
### CMP-003 frozen process checkpoint

The frozen 91-case executable authority appends `CMP-003`. `request CMP-003` emits one bounded private-free context
...
collar radius exits `2` as prepublication `invalidSubmission`. Schema v8,
command surface, byte bound, and exit mapping remain
...
### CMP-004 frozen process checkpoint

The frozen 92-case executable authority appends `CMP-004`. `request CMP-004` emits one bounded private-free context
with ordered roles `plate`, `pin-a`, `pin-b`. The plate is a
200 mm x 100 mm x 10 mm box at the origin. Both pins are cylinders with radius
8 mm and depth 50 mm along +Z, based at `(25, 25, 10)` mm and
`(175, 25, 10)` mm respectively. Exact candidate-response v8 documents supplied
by file and standard input execute one atomic production batch and exit `0`
with `realized`.

Swapping the public pin payloads while preserving role and member order exits
`2` as postpublication `invalidSubmission` without retry. A zero radius on the
third `pin-b` member exits `2` as prepublication `invalidSubmission`;
Schema v8, command surface, byte bound, and exit mapping remain unchanged. The known 91-request aggregate remains
`c3da3fd9024e8a568da27b9961abbd06b6d1e99dab621d297af8b8f222f07813`;
...
respectively. Exact candidate-response v8 documents supplied by file and
standard input execute one atomic production batch and exit `0` with
`realized`.
...
### CMP-006 frozen process checkpoint

The frozen 94-case executable authority appends `CMP-006`.
`request CMP-006` emits one bounded private-free context with ordered roles
`hub`, `arm-a`, `arm-b`. The hub is a cylinder with radius 20 mm and depth
50 mm at the origin along +Z. Both arms are 100 mm x 10 mm x 10 mm boxes
based at `(20, -5, 20)` mm and `(-120, -5, 20)` mm respectively. Exact
candidate-response v8 documents supplied by file and standard input execute
one atomic production batch and exit `0` with `realized`.
...
third `arm-b` member exits `2` as prepublication `invalidSubmission`. Schema
v8, command surface, byte bound, and exit mapping remain unchanged. The known
93-request aggregate remains
`bcb6db981a67668a16a18c2654ec2b6465668d7a8396761bfc3bacdcbe7d67dc`;
...
### CMP-007 frozen process checkpoint

The frozen 95-case executable authority appends `CMP-007` as the final
compound case. `request CMP-007` emits one bounded private-free context with ordered
roles `block`, `bore`. `block` is a 50 mm cube at the origin. `bore` is an
independent positive cylinder with radius 10 mm and depth 50 mm at
`(25, 25, 0)` mm along +Z; no boolean subtraction is part of this case.
Exact candidate-response v8 documents supplied by file and standard input
execute one atomic production batch and exit `0` with `realized`.

Moving the public `bore` base to x=26 mm exits `2` as postpublication
`invalidSubmission` without retry. A zero radius or zero axis on `bore` exits
`2` as prepublication `invalidSubmission`.
Schema v8, command surface, byte bound, and exit mapping remain unchanged. The
known 94-request aggregate remains
`978ee6b0ac70c3d90a87b0f44fb634d4aec0fbaf42126db9ed8721bec7431c43`;
...
### SPH-002 frozen process checkpoint

The frozen 97-case executable authority appends `SPH-002`.
`request SPH-002` emits one bounded private-free context for
...
### SPH-003 frozen process checkpoint

The frozen 98-case executable authority appends `SPH-003`.
`request SPH-003` emits one bounded private-free context for
a radius-0.1 m analytic sphere centered at `(0, 0, 0.1)` m. Its public
capability snapshot reports `cad.solid.analytic-sphere@1` as unavailable with
reason `not-exposed`.
...
Schema v8, command surface, byte bound, and exit mapping remain unchanged. The
frozen 97-request aggregate remains
`c92d00ee7d48e3be11aa0d3880bb21f4998b8c037d35a838f0c1a1a691eea66c`;
appending the actual `SPH-003` request freezes the 98-request aggregate as
`4e7b748413486562399baecf01d4076bd3c90df281174c0bd7226bd0f9f1e552`.

### SPH-004 frozen process checkpoint

The frozen 99-case executable authority appends `SPH-004`.
`request SPH-004` emits one bounded private-free context for
a radius-2 in analytic sphere centered at `(-2, 3, 1)` in. Its public
capability snapshot reports `cad.solid.analytic-sphere@1` as unavailable with
reason `not-exposed`.
...
Schema v8, command surface, byte bound, and exit mapping remain unchanged. The
frozen 98-request aggregate remains
`4e7b748413486562399baecf01d4076bd3c90df281174c0bd7226bd0f9f1e552`;
...
production executor and exits `2` with `expectedUnsupported`. A substitute box
action or generic `capabilityUnavailable` declaration exits `2` with
`invalidSubmission`; no reference action or synthetic sphere is executed. The
activated authority has count 100 and set equality with the full catalog;
executor category order and catalog lexical order remain separate contracts,
and there is no fictitious inactive case ID.

Schema v8, command surface, byte bound, and exit mapping remain unchanged. The
frozen 99-request aggregate remains
`c3002db14457157163c12c6db9e2e3794ba330476984ec1321c0212b45d02a78`;
appending the actual `SPH-005` request freezes the 100-request aggregate as
